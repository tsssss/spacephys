...
pro fig_data_coverage_spherical_hydra, bininfos=bininfos, tdat=tdat, bincnts=bincnts, noplot=noplot, slicerng=slicerng, _extra=extra
...
;---Settings.
    ...
    sliceidx = 0
    dmlt = 2
    if n_elements(slicerng) eq 0 then slicerng = [-1,1]*dmlt+12
    ...
    case sliceidx of
        0: begin
            xtitl = 'X SM (Re)'
            ytitl = 'X SM (Re)'
            ztitl = 'MLT in ('+sgnum2str(mean(slicerng))+','+sgnum2str(mean(slicerng)-12)+')+/-'+sgnum2str(dmlt)+' hr!CCount per bin'
            xrang = [-1d,1]*10
            yrang = [0d,1]*10
            zrang = [0d,5]
            minlat = 45d
...
;    stop
    ...
    fig_data_coverage_spherical_hydra, bininfos=bininfos, tdat=tdat, bincnts=bincnts, /noplot, slicerng=slicerng-12
    for i=0,tsz[0]-1 do for j=0,tsz[1]-1 do begin
        tc = tdat[i,j]
        if tc le zrang[0] then continue
