...
function fig_2015_0312_0900_dmsp_overview_v02, plot_file, event_info=event_info, test=test
...
test=1
    ...
    ssusi_time = time_double('2015-03-12/09:05:39')
    asi_time = ssusi_time
    time_range = time_double(['2015-03-12/08:45','2015-03-12/09:10'])
    dmsp_orbit_time_range = time_double(['2015-03-12/09:02','2015-03-12/09:08'])    ; for plotting orbit.
    dmsp_plot_time_range = time_double(['2015-03-12/09:04','2015-03-12/09:07'])     ; for tplot.
    invertedv_time = time_double('2015-03-12/09:05:42')
...
;    ssusi_id = 'lbhl'
    ...
    endif else ssusi_wavelength = ssusi_id
...
;db_var = dmsp_read_bfield(time_range, probe=probe)
...
;            xyouts, tx,ty,normal=1, msg, charsize=label_size, color=sgcolor('red')
    ...
    xyouts, tx1+xchsz*0.5, ty1-ychsz*0.2, normal=1, 'DMSP F19', charsize=sc_label_size, color=dmsp_color
...
;---DMSP panels.
    ...
    ztickn = '10!U'+string([5,6,7,8,9],format='(I0)')
    ztickn[0:*:2] = ' '
    ...
    tmp = convert_coord(invertedv_time,0, data=1, to_normal=1)
    tx = tmp[0]
    ty = tpos[1]+ychsz*0.2
    msg = invertedv_text
    xyouts, tx,ty,normal=1, msg, charsize=sc_label_size, alignment=0.5, color=invertedv_color
    tys = [ty+ychsz*1, tpos[1]+(tpos[3]-tpos[1])*0.6]
    plots, tx+[0,0],tys, normal=1, color=invertedv_color
    plots, tx,tys[1],normal=1, psym=8, symsize=0.5, color=invertedv_color
...
;---Auroral snapshots.
    ...
    xids = findgen(npx)
    stop
    ; scan each column for bad data.
    for ii=0,npx-1 do begin
        tmp = reform(mlt_image[ii,*])
        dtmp = deriv(tmp)
        flags = dtmp[1:npx-1] eq dtmp[0:npx-2]
        index = where(flags eq 1, count)
        if count eq 0 then continue
        index_ranges = time_to_range(index,time_step=1)
        index_ranges[*,1] += 1
        nindex_range = n_elements(index_ranges[*,0])
        for jj=0,nindex_range-1 do begin
            the_range = reform(index_ranges[jj,*])
            if the_range[0] eq 0 then continue
            if the_range[1] eq npx-1 then continue
            tmp[the_range[0]:the_range[1]] = !values.f_nan
        endfor
        mlt_image[ii,*] = tmp
    endfor
    stop
    ; interpolate bad data for each row.
    for ii=0,npx-1 do begin
        tmp = reform(mlt_image[*,ii])
        index = where(finite(tmp,nan=1), count, complement=index2)
        if count eq 0 then continue
        tmp = interpol(tmp[index2], xids[index2], xids)
        mlt_image[*,ii] = tmp
    endfor
;    for ii=0,npx-1 do begin
;        tmp = reform(mlt_image[*,ii])
...
;        for jj=0,nindex_range-1 do begin
;            the_range = reform(index_ranges[jj,*])
;            duration = abs(total(the_range*[-1,1]))
;            if duration ge 5 then continue
;            if the_range[0] eq 0 then continue
;;            if duration eq 0 then tmp[the_range[0],ii] = !values.f_nan else tmp[the_range[0]:the_range[1],ii] = !values.f_nan
;            tmp[the_range[0]:the_range[1]] = !values.f_nan
;        endfor
;        index = where(finite(tmp,nan=1), count, complement=index2)
;        if count eq 0 then continue
...
;---RBSP.
    ...
    foreach the_info, aurora_info do begin
        ...
        xyouts, tx-xchsz*0.5,ty+ychsz*1,normal=1, 'RBSP-B', color=sc_color, charsize=sc_label_size
    endforeach
...
test = 0
print, fig_2015_0312_0900_dmsp_overview_v02(event_info=event_info, test=test)
end
