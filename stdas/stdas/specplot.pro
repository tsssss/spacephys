...
; Sheng.
str_element, opt, 'color_table', value=ct
if n_elements(ct) eq 0 then ct = 40
str_element, opt, 'reverse_color_table', value=reverse_ct
if n_elements(reverse_ct) eq 0 then reverse_ct = 0
...
str_element, opt, 'zhorizontal', zhorizontal
...
if not keyword_set(no_color_scale) then begin
  if keyword_set(bottom) and keyword_set(top) then begin
    draw_color_scale,brange=[bottom,top],range=zrange,log=zlog,title=ztitle, $
      ...
      horizontal=zhorizontal, color_table=ct, reverse_ct=reverse_ct
  endif else begin
