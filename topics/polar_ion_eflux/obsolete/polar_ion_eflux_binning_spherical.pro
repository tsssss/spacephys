;+
; Bin data based on spherical coord: mlt, mlat, and dis.
;-


pro polar_ion_eflux_binning_spherical, utr0
...
;---Settings.
    ...
    logfn = rootdir+'/polar_ion_eflux_binning_spherical_'+time_string(utr0[0],tformat='YYYY')+'.log'
    if file_test(logfn) then file_delete, logfn & stouch, logfn
    datfn = rootdir+'/polar_ion_eflux_bin_info_spherical_'+$
    ...
    posvars = ['Epoch','EDMLT_TIME','MAG_LATITUDE','GSE_POS']
...
;---Loop through each day.
    ...
    for i=0, nday-1 do begin
        ...
        free_lun, lun
        pos = sread_polar_orbit(tutr, flag='def', vars=posvars, /local_only)
        if size(pos,/type) ne 8 then pos = sread_polar_orbit(tutr, flag='pre', vars=posvars, /local_only)
        ...
        ;---read position, and map it to the ionosphere.
            ...
            rdis = snorm(pos.gse_pos)*re1  ; in-situ dis in Re.
            rdis = sinterpol(rdis, tuts, uts)
            rmlt = sinterpol(pos.edmlt_time, tuts, uts)    ; in hr in [0,24].
            mlat = sinterpol(pos.mag_latitude, tuts, uts)
            mlat = abs(mlat)
                ...
                for k=0, nbinlat-1 do begin
                    for l=0, nbindis-1 do begin
                        ...
                        idx = where(rdis ge tdisrng[0] and rdis lt tdisrng[1] $
                            and mlat ge tlatrng[0] and mlat lt tlatrng[1] $
...
;polar_ion_eflux_binning_spherical, time_double(['1996-11-12','1996-11-13'])
;stop
;polar_ion_eflux_binning_spherical, time_double(['1996-03-17','1996-12-31'])
polar_ion_eflux_binning_spherical, time_double(['1997-01-01','1997-12-31'])
polar_ion_eflux_binning_spherical, time_double(['1998-01-01','1998-12-08'])
end
