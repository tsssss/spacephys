...
pro fig1_example_event_with_ion_vel, event, project=project, $
...
;---Get the size of the figure, and position of the panels.
    ...
    file = join_path([project.plot_dir,'paper_events','fig_'+event_id+'_timing_result_with_ion_vel.pdf'])
...
;---Draw ion_vel.
    ion_vel_color = sgcolor('salmon')
    min_vmag = 10.
    foreach df, df_list do begin
        probe = df.probe
        if strmid(probe,0,2) ne 'th' then continue
        the_probe = strmid(probe,2,1)
        the_time_range = df.obs_time+[-1,1]*15*60
;        themis_read_ion_vel, the_time_range, probe=the_probe
;        the_var = probe+'_u_gsm'
;        thm_load_mom, probe=the_probe, trange=the_time_range, use_eclipse_corrections=2, suffix='_corr'
        thm_load_mom, probe=the_probe, trange=the_time_range, level='l2'
        the_var = probe+'_peim_velocity_gsm'
        u_gsm = get_var_data(the_var, in=the_time_range, times=times)
        ...
        u_sm = total(u_sm,1)/ntime
        vel = u_sm[0:1]
        v_hat = sunitvec(vel)
        v_mag = snorm(vel)
        if v_mag le min_vmag then continue
        ...
        x1 = x0+v_hat[0]*scale
        y1 = y0+v_hat[1]*scale
        arrow, x0,y0,x1,y1,/data, solid=arrow_solid, hsize=hsize, color=ion_vel_color
    endforeach
...
;---Select test_list.
    ...
    foreach candidate, test_candidates do begin
        fig1_example_event_with_ion_vel, candidate
    endforeach
end
