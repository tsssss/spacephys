...
;---Calc PSD.
    ...
    foreach vars, var_list do begin
        ...
        for ii=0,nsector-1 do begin
            ...
            if count gt 0.1*ndata then continue
            ...
            index_lim = 3
            if max_index lt index_lim then continue ; Too close to the low freq.
            if max_index gt n_elements(index)-index_lim then continue ; Too close to the high freq.
        ...
        index = strpos(line_var, 'kev_e_flux')
        if index[0] ne -1 then begin
            options, line_var, yrange=[0,5], constant=[1.5]
        ...
        plot_tr = time_double(time_range)
        ...
        for ii=0,nsector-1 do begin
            ...
            tr = psd_times[ii:ii+1]
            plots, tr, [0,0], color=sgcolor('red'), thick=thick, data=1
        endfor
        stop
