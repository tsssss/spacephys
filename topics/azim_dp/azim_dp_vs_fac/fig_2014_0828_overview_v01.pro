...
test = 0
    ...
    plot_dir = join_path([homedir(),'Dropbox','mypapers','dp_vs_fac'])
    file = join_path([plot_dir,'fig_2014_0828_overview_v01.pdf'])
...
;---Get the size of the figure, and position of the panels.
    ...
    ; Get the sizes of left panels.
    margins = [12.,5,8,2]
    panel_xskip = 8
    panel_yskip = 4

    right_pan_xsize = long_pan_xsize*0.5
    right_pan_ysize = right_pan_xsize
    ...
    sgopen, file, xsize=fig_xsize, ysize=fig_ysize, /inch, xchsz=xchsz, ychsz=ychsz
    ...
    lower_pos = tposs[*,1]
    long_pan_poss = sgcalcpos(lower_nypanel, position=lower_pos, ypans=long_ypans, ypad=lower_panel_ypad)
...
;---Panel b. UT-MLT long.
    ...
    tx = label_xshift*xchsz
    ty = tpos[3]-label_yshift*ychsz
    xyouts, tx,ty,/normal, 'c. Scaled '+tex2str('theta')
...
;---Right panel.
    ...
    tx = tpos[0]
    ty = tpos[3]+ychsz*0.5
    label = 'b. Equatorial (SM XY) plane'
