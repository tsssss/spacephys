function micro_injection_themis_2009_0619_fig_thd_the_v02, test = test
    ...
    yrange = [16.5, 10]
    plot_dir = srootdir()
    base = 'micro_injection_themis_2009_0619_fig_thd_the_v02.pdf'
...
;---Right panels.
    ...
    foreach probe, probes do begin
        prefix = 'th' + probe + '_'
        my_info = info[probe]
        time_range = my_info['time_range']+[-1,1]*1200d
...
;        endif
    ...
    corr_var = 'b'+the_component+'_corr'
    plot_vars = [plot_vars,corr_var]
    labels = [labels,'B'+the_component+' corr']
    nplot_var = n_elements(plot_vars)
    ypans = [ypans,0.4]

    vars = 'th'+probes+'_b_gsm'
    corr_tr = time_double(['2009-06-19/01:30','2009-06-19/03:50'])
    corr_range = [-1,1]*600
    ...
    foreach bar_time, bar_times do begin
        ...
        foreach probe, probes, probe_id do begin
            ...
            tys = [tys, tmp[1]]
        endforeach
        hsize = keyword_set(test)? 4: 160
        thick = keyword_set(test)? 1: 4
        arrow, txs[0],tys[0],txs[1],tys[1],normal=1, color=color, $
            linestyle=linestyle, hsize=hsize, solid=1, thick=thick
    endforeach
    ...
    if count ne 0 then begin
        ...
        tx = tpos[2]-xchsz*0.5
        ty = tpos[1]+ychsz*0.3
        msg = 'TH-E shifted by '+string(corr_dt, format='(I0)')+' sec'
...
test = 0
print, micro_injection_themis_2009_0619_fig_thd_the_v02(test = test)
end
