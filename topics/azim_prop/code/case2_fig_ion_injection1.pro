...
;---Settings.
    ...
    test_time_range = time_double(['2014-08-28/10:15','2014-08-28/10:25'])
    left_label = 'f'
    right_label = 'g'
...
;---Get the flux in the wanted energy range, and the drift periods.
    probes = ['thd','LANL-01A','LANL-02A','LANL-04A']
...
;if ~keyword_set(test) then if the_test_time eq plot_time then continue
        ...
        foreach test_mlt, test_mlts, jj do begin
            ...
            drift_periods = abs(get_var_data(drift_period_var, at=times))
            new_fluxes = fluxes
...
;---Caclulate the MLT of dispersionless injection.
    ...
    foreach probe, probes, probe_id do begin
        ...
        ; Get drift period.
        drift_period_var = prefix2+'drift_period'
        drift_periods = abs(get_var_data(drift_period_var, at=new_times))
...
;    endforeach
    ...
    ofn = join_path([root_dir,'case2_fig_ion_injection1.pdf'])
    ...
    ty = tpos[3]+ychsz*0.5
    xyouts, tx,ty,/normal,alignment=0.5, 'Ion flux (#/cm!U2!n-s-sr-keV) in real time'
