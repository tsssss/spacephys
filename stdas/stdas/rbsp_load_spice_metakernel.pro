...
if keyword_set(all) then begin
	...
	if n_elements(utr) ne 0 then tr = utr
endelse
...
while not eof(unit) do begin
  ...
  if inblock then begin
    ...
    if strpos(kerneltoload,'attitude_history/') ne -1 then begin
		...
		; skip unwanted probe.
        if n_elements(probe) eq 1 then $
            if strpos(file_basename(strlowcase(kerneltoload)),'rbsp'+strlowcase(probe[0])) eq -1 then continue
    ...
    if kerneltoload ne '' then begin
      ...
      if file_test(files[0]) ne 0 then begin    ; fix bug, when file list is updated before files ...
