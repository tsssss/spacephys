...
function tracers_read_ion_pa_spec, input_time_range, probe=probe, $
...
;    anode_dirs = -anode_dirs ; The flux direction is opposite to the anode direction.
    ...
    pa_spec_var = prefix+'ion_pa_spec'+suffix
    ...
    en_spec_var = prefix+'ion_en_spec'+suffix
    ...
    options, plot_vars, zrange=[1e5,1e10]*1e-2
;    tplot, plot_vars
;    stop
...
probe = '2'
pad_var = tracers_read_ion_pa_spec(time_range, probe=probe)
pad_var = tracers_read_ele_pa_spec(time_range, probe=probe)
mlat_vars = tracers_read_mlat_vars(time_range, probe=probe)
prefix = 'ts'+probe+'_'
suffix = '_cdaweb'
;plot_vars = prefix+[['ion','ele']+'_pa_spec',['ion','ele']+'_en_spec']+suffix
plot_vars = prefix+['ion_'+['en_spec','pa_spec'],'ele_'+['en_spec','pa_spec']]+suffix
nplot_var = n_elements(plot_vars)
labels = letters(nplot_var)+'. '+['Ion '+['EN','PA'],'Ele '+['EN','PA']]
label_vars = prefix+['mlt','mlat']
nlabel_var = n_elements(label_vars)
options, prefix+'mlt', ytitle='MLT (h)'
options, prefix+'mlat', ytitle='MLAT (deg)'

test = 0
plot_file = join_path([googledir(),'works','2026_tracers_ion','test_pad_'+time_string(time_range[0],tformat='YYYY_MMDD_hh')+'_v01.pdf'])
if keyword_set(test) then plot_file = 0
fig_size = [6,6]
margins = [14,4+nlabel_var,10,2]
vlab_margin = 2
poss = sgcalcpos(nplot_var, margin=margins)
sgopen, plot_file, size=fig_size, xchsz=xchsz, ychsz=ychsz
tplot, plot_vars, trange=time_range, position=poss, var_label=label_vars, vlab_margin=margins[0]-vlab_margin
for pid=0,nplot_var-1 do begin
    tpos = poss[*,pid]
    tx = xchsz*vlab_margin
    ty = tpos[3]-ychsz*0.75
    msg = labels[pid]
