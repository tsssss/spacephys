...
pro rbsp_efw_find_l1_time_tag_irregularity, probe=probe, filename=file, errmsg=errmsg
...
;---The L1 data are supposed to be available on the local disk.
    ...
    ;---Find the mission time range and prepare the files for each day.
        time_range = rbsp_efw_phasef_get_valid_range(l1_type+'_l1', probe=probe)
...
foreach probe, ['a','b'] do begin
    rbspx = 'rbsp'+probe
    file = join_path([root_dir,rbspx,'l1','time_tag_correction',rbspx+'_l1_time_tag_irregularity.cdf'])
    rbsp_efw_find_l1_time_tag_irregularity, probe=probe, filename=file, errmsg=errmsg
endforeach
end
