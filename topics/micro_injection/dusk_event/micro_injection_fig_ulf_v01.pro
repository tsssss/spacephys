...
function micro_injection_fig_ulf_v01, input_event_id, probe=probe, $
...
;---Pressure.
    ...
    pth_var = lets_calc_pthermal(n_var=prefix+'p_density_fpi',t_var=tavg_var)
    pcombo_var = prefix+'p_combo'
    pth = get_var_data(pth_var, times=times)
    pb = get_var_data(pmag_var, at=times)
    ptot = pth+pb
    store_data, pcombo_var, times, [[ptot],[pth],[pb]]
    add_setting, pcombo_var, smart=1, dictionary($
        ...
        'labels', 'P!D'+['tot','th','B'], $
        'colors', constant('rgb') )
    options, pcombo_var, yrange=[0,0.8], ystyle=1
...
;---Plot settings.
    ...
    plot_vars = prefix+[$
        ...
        'b_gsm_plot','e_mms_fac','vexb_mms_fac','u_mms_fac', $
        'e_density_fpi','p_t_fpi','p_combo']
    panel_labels = [$
        'Ele EN high','Ele EN',$
        'B GSM','E FAC','V!DExB!N FAC','V!Dion!N FAC', $
        'N','T','P']
    
    
    plot_time_range = time_double(['2015-09-01/18:00','2015-09-01/21:30'])
    plot_time_range = time_double(['2015-09-01/15:00','2015-09-01/17:00'])
    tplot_options, 'tickinterval', 1200
    ;plot_time_range = time_range
    ;tplot_options, 'tickinterval', 1200*4
    pansize = [6,0.75]
    fig_info = sgplot(plot_vars, panel_labels=panel_labels, $
        xrange=plot_time_range, filename=plot_file, pansize=pansize)
...
print, micro_injection_fig_ulf_v01(event_id, probe=probe, test=1, update=1)
end