...
; Time-lagged, scaled separately.
; Color-coded with discrete colors.
;-

pro azim_prop_plot_all_data5, project
...
;---Plot settings.
    ...
    file = join_path([project.plot_dir,'fig_all_'+data_type+'5.pdf'])
...
;---Panels.
    ...
    foreach event_id, sorted_events do begin
        ...
        ty_label = tpos[1]-ychsz
        probes = sorted_probes[sort(sorted_probes)]
        nprobe = n_elements(probes)
        strlength = nprobe*3
        tx_label = (tpos[0]+tpos[2])*0.5-xchsz*0.5*strlength
        ref_time0 = event.ref_time
        ...
        foreach probe, sorted_probes, ii do begin
            ...
            ty = tpos[3]
            tmp = convert_coord(ref_time,0, /data, /to_normal)
            tx = tmp[0]
            arrow, tx,ty+0.4*ychsz, tx,ty, /normal, color=color, hsize=arrow_hsize*2, /solid, thick=data_thick*2

            ; Add y-labels for the yrange.
            tx = xchsz*2
            ty = mean(tpos[[1,3]])-ychsz*0.3*label_size
            xyouts, tx,ty,/normal, sgnum2str(common_scale)+' '+var_unit, charsize=label_size
            tx = tpos[0]-xchsz*4.5+xchsz*ii*0.75
            ...
            xyouts, tx_label,ty_label,/normal, sc_short, color=color
            tx_label += strlen(sc_short)*xchsz
        endforeach
        
        if event_id eq sorted_events[0] then begin
            tmp = convert_coord(ref_time0,0, /data, /to_normal)
            tx = tmp[0]-xchsz*1
            ty = tpos[3]
            xyouts, tx,ty,/normal, alignment=1, 'time lag', charsize=label_size
        endif
        ...
        if event_id eq sorted_events[0] then begin
            ty = ty+ychsz*label_size
            xyouts, tx,ty,/normal, alignment=0, 'azim scale', charsize=label_size, color=scale_color
        endif
