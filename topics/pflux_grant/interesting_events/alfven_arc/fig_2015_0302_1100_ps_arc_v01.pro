...
function fig_2015_0302_1100_ps_arc_v01, plot_file, event_info=event_info
...
test = 0
    ...
    bar_times = time_double('2015-01-05/'+['00:41','01:09'])
...
;---plot_vars options
    ...
    center_mlon = site_info.asc_mlon
    center_mlts = mlon2mlt(center_mlon, times)
    index = where(center_mlts ge 12, count)
    if count ne 0 then center_mlts[index] -= 24
    foreach time, times, time_id do begin
        ;the_mlt_range = center_mlts[time_id]+[-1,1]*dmlt-0.5
        the_mlt_range = fmlts[time_id]+[-1,1]*dmlt+0.1
    ...
    yminor = 5
    index = where(hehe eq 0, count)
    if count ne 0 then hehe[index] = !values.f_nan
    ...
    var = prefix+'p_en_spec'+['','_'+['para','perp','anti']]
    options, var, 'zrange', zrange
    options, var, 'ztickv', ztickv
    options, var, 'zticks', zticks
    options, var, 'zcharsize', label_size
    ...
    var = prefix+'o_en_spec'+['','_'+['para','perp','anti']]
    options, var, 'zrange', zrange
    options, var, 'ztickv', ztickv
    options, var, 'zticks', zticks
    options, var, 'zcharsize', label_size
    ...
    var = pf_var
    options, var, 'yrange', [-120,60]
    options, var, 'ytickv', [-100,-50,0,50]
...
;---Plot.
    sgopen, plot_file, size=[6,6], xchsz=xchsz, ychsz=ychsz
    margins = [12,4,10,1]
    ...
    if count ne 0 then begin
        ...
        oplot, times, fmlats[*,model_index]+dmlat, color=rbsp_color, linestyle=2
        tx = time_range[0]
    ...
    if count ne 0 then begin
        ...
        get_data, prefix+'e_t', times, data
        oplot, times, data
    endif
...
print, fig_2015_0302_1100_ps_arc_v01(event_info=event_info)
end
