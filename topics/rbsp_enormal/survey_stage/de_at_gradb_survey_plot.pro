...
pro de_at_gradb_survey_plot, utr, tprobe
    
    rootdir = shomedir()+'/de_at_gradb'
    pre0 = 'rbsp'+tprobe+'_'
    deg = 180d/!dpi
    rad = !dpi/180
    re = 6378d & re1 = 1d/re
...
;    idx = where(abs(dat[*,2]) ge 100, cnt)
;    if cnt ne 0 then dat[idx,2] = !values.d_nan
    dat[*,2] = 0
...
; **** map footprint.
    ...
    vars = pre0+['de_mgse','de_svy','b_gse','b','e_en','h_en','o_en','n','t','beta']
    ...
    ofn = rootdir+'/de_at_gradb_'+time_string(utr[0],tformat='YYYY_MMDD_hh')+'_rbsp'+tprobe+'.pdf'
    sgopen, ofn, xsize = 8.5, ysize = 11, /inch
    device, decomposed = 0
    loadct2, 43
...
rootdir = shomedir()+'/Google Drive/works/works/rbsp_de/round3_2016_0707/de_at_gradB'
fns = file_search(rootdir+'/*.pdf')
nfn = n_elements(fns)

for i = 0, nfn-1 do begin
    tfn = file_basename(fns[i])
    tprobe = strmid(tfn,4,1)
    t0 = strmid(tfn,15,14)
    ut0 = time_double(t0, tformat='YYYY_MMDD_hhmm')
    utr = ut0+[-1,1]*15*60
    de_at_gradb_survey_plot, utr, tprobe
endfor
