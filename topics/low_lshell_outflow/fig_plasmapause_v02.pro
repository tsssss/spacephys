...
function fig_plasmapause_v02_right_panels, right_pos, event_info=event_info
    ...
    ypans = fltarr(nvar)+1
    ypans[0] = 1.2
    ypads = fltarr(nvar-1)+1d & ypads[0] = 0.4
    right_poss = sgcalcpos(nvar, position=right_pos, ypans=ypans, ypad=ypads)
    ...
    tplot_options, 'tickinterval', 3600d*6
    ...
    foreach pp_info, pp_infos do begin
        ...
        ; Add vertical line.
        tpos = right_poss[*,-1]
        tpos[3] = right_poss[3,0]
        xr = time_range
        yr = [0,1]
        plot, xr, yr, $
            xrange=xr, yrange=yr, xstyle=5, ystyle=5, $
            position=tpos, nodata=1, noerase=1
        plots, pp_time+[0,0], yr, linestyle=1, color=sgcolor('gray')
        ...
        ; Add time.
        msg = time_string(pp_time,tformat='hh:mm')
        tmp = convert_coord(pp_time,yr[0], data=1, to_normal=1)
        tx = tmp[0]
