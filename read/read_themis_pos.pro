...
pro read_themis_pos, utr0, probe=probe, newname=newname, $
...
;---prepare var names.
    pre0 = 'th'+prb+'_'
    var0s = ['Epoch','XYZ_GSM']   ; 3 sec resolution.
...
;---Move data to a structure then to tplot.
    ...
    re = 6378d  ; km.
    tuts = sfmepoch(dat.(0),'unix')
    rgsm = sinterpol(dat.(1), tuts, ut0s)   ; in Re.
    
    myvars = pre0+['pos_gsm']
    if not keyword_set(newname) then newname = 'pos_gsm'
    store_data, pre0+newname, ut0s, rgsm, $
        limits = {ytitle:'(Re)', colors:[6,4,2], labels:'GSM R'+['x','y','z'], labflag:-1}
