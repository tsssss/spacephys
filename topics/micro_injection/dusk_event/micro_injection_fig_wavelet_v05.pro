...
; end up with not adding 95% line b/c it is too small.
; 
;-
function micro_injection_fig_wavelet_v05, input_event_id, probe=probe, $
...
;---Load data.
    ...
    var = lets_read_this(func='mms_read_pad_ele_kev', $
        time_range, probe=mission_probe, errmsg=errmsg, version='v01')
...
;---Load more data
    ...
    foreach tid, [0,1] do begin
        ...
        unit = settings.unit
        ztitle = 'e- flux [Log!D10!Nflux]!U2'
        yrange = [0.3,30]
...
;        stop
;        tys = cwt_info.ngsignif/ty0
;        oplot, txs,tys, color=color, linestyle=1
...
print, micro_injection_fig_wavelet_v05(event_id, probe=probe, test=0, update=1)
end