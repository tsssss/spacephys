...
function fig_rbsp_overview_v01, test=test, event_info=event_info
    ...
    time_range = time_double(['2015-01-04/11:40','2015-01-04/12:50'])
    plot_time_range = time_range
    probes = ['b']
    
    ob_times = time_double('2015-01-04/'+['11:52','12:20','12:40'])
...
;        o_pa_var = rbsp_read_pa_spec(time_range, probe=probe, species='o', errmsg=errmsg, energy_range=[50,5e4], update=update)
        ...
        e_vars = e_vars[1:2]
        var = e_vars
        options, var, 'yrange', [-1,1]*10
        options, var, 'ytickv', [-1,0,1]*6
        options, var, 'yticks', 2
        options, var, 'yminor', 3
        options, var, 'constant', [-1,0,1]*5
        fac_labels = ['||',tex2str('perp')+','+['west','out']]
        options, var[0], 'labels', 'E!D'+fac_labels[1]
        options, var[1], 'labels', 'E!D'+fac_labels[2]
        ...
        model_suffix = '_dipole_t89_north'
        ...
        fmlt_var = prefix+'fmlt_dipole_t89_north'
        ...
        fmlat = get_var_data(fmlat_var, at=times)
        fmlat_del1 = [0.5,3]
        fmlat_del2 = [-1,-0.5]
        fmlat_del = [-0.5,3]
        ...
        zlim, ewo_var, 2e3,8e3, 0
        options, ewo_var, ztickv=[3,5,7]*1e3, zticklen=-0.5, $
            zticks=2, zminor=4, ztickname='10!U'+['3','5','7'], ztitle='Count (#)'
        
        db_var = prefix+'b1_fac'
        
        vars = [theta_combo_var,e_en_var,p_en_var,o_en_var,$
            e_var,e_spec_var,fmlat_var]
        vars = [ewo_var,e_vars,e_spec_combo]
        nvar = n_elements(vars)
        fig_letters = letters(nvar)
        fig_labels = fig_letters+') '+['Ewo','E!D'+fac_labels[1:2]+'!N','E spec']
        ypans = fltarr(nvar)+1.
        index = where(vars eq e_spec_combo, count)
        if count ne 0 then ypans[index] = 1.6
        index = where(vars eq ewo_var, count)
        if count ne 0 then ypans[index] = 1.6
        ...
        for ii=0,nvar-1 do begin
            ...
            spec = get_setting(vars[ii],'spec',exist)
            if exist and spec eq 1 or vars[ii] eq e_spec_combo then begin
                cbpos = tpos
                cbpos[0] = cbpos[2]+xchsz*0.8
                cbpos[2] = cbpos[0]+xchsz*0.8
                options, vars[ii], 'zposition', cbpos
            endif
        endfor
        ...
        tplot_options, 'tickinterval', 10*60
...
;        xyouts, tx,ty,msg, normal=1;, color=sgcolor('white')
        ...
        pid = where(vars eq ewo_var, count)
        if count ne 0 then begin
            tpos = poss[*,pid]
