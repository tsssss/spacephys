...
pro azim_df_fig_df_distr4, project=project

test = 0
    if n_elements(project) eq 0 then project = azim_df_load_project()
    magnify = keyword_set(test)? 1.5: 1
    plot_file = join_path([project.plot_dir,'fig_df_distr4.pdf'])
...
;        sgnum2str(fit_coeff[0]/fit_coeff[1],ndec=1)+'+Rxy)'
    index = where(width_rxy_stddevs gt 10)
    fit_coeff = mean(rxy_bin_centers[index]*width_rxy_stddevs[index]/60*width_mag)
    xxs = rxy_bin_centers
