;+
; Calculate the drift velocity for a given particle at certain energy, pitch angle, and location,
...
; return the drift velocity in deg/sec.
;-
;


function calc_drift_velocity, rgsm=rgsm0, time=ut0, $
    energy=energy0, pitch_angle=pitch_angle, species=species, $
    model=model, duration=duration, bounce_period=bounce_period
...
;---Settings.
    ...
    if n_elements(pitch_angle) eq 0 then message, 'no pitch angle ...' & pitch_angle0 = pitch_angle*rad
    if n_elements(model) eq 0 then model = 'igrf'
    if n_elements(duration) eq 0 then duration = 2d*(400d/energy0) ; sec, inverse scaled with energy.
...
foreach energy0, energys do begin
    v_drift = calc_drift_velocity(rgsm=rgsm0, time=ut0, $
