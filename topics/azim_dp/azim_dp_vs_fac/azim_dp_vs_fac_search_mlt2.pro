...
function azim_dp_vs_fac_search_mlt2, roi_list, $
    reset=reset
...
;---Check input.
    ...
    if n_elements(mlt_max_sep) eq 0 then mlt_max_sep = 2.5   ; hr
    ;mlt_max_avg_sep = 1.5   ; hr, mlt avg separation must be smaller than this.
    roi_min_duration = 1800.    ; sec.
    min_probe_count = 4
...
;---Settings.
    ...
    log_file = join_path([root_dir,'azim_dp_vs_fac_search_mlt2.log'])
    out_file = join_path([root_dir,'azim_dp_vs_fac_search_mlt2.txt'])
...
mlt_list = azim_dp_vs_fac_search_mlt2(roi_list)
