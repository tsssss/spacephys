...
pro psbl_de_update_info, ilogfn, infofn, save_plot = save_plot

    if n_elements(ilogfn) eq 0 then $
        ilogfn = shomedir()+'/Google Drive/works/works/rbsp_de/'+$
            'dipolarization/list_large_de_round3.log'

    if n_elements(infofn) eq 0 then $
    ...
    for i0 = 0, ninfo-1 do begin
        ...
        infos[i0].tsta = t1
        infos[i0].tend = t2
        utr = mean([t1,t2])+[-0.5,0.5]*dt   ; larger time range for pflux calc.
        ...
        ; set jumpindensity, jumpinbeta.
        get_data, pre0+'n', t0, dat
        idx = where(t0 ge utr[0] and t0 le utr[1], cnt)
        if cnt ne 0 then infos[i0].jumpindensity = max(dat[idx])/min(dat[idx])
        get_data, pre0+'beta', t0, dat
        idx = where(t0 ge utr[0] and t0 le utr[1], cnt)
        if cnt ne 0 then infos[i0].jumpinbeta = max(dat[idx])/min(dat[idx])
        ...
        get_data, 'dst', t0, dat
        idx = where(t0 ge utr[0] and t0 le utr[1], cnt)
        infos[i0].dst = (cnt eq 0)? !values.d_nan: min(dat[idx])
        get_data, 'ae', t0, dat
        idx = where(t0 ge utr[0] and t0 le utr[1], cnt)
        infos[i0].ae = (cnt eq 0)? !values.d_nan: max(dat[idx])
        ...
        infos[i0].efac = ttinfo[0]
        infos[i0].edot0fac = ttinfo[1]
        
        ; set mlt, fptmlt, fptmlat.
        deut = infos[i0].efac.ut
        if deut eq 0 then begin
            infos[i0].mlt = !values.d_nan
            infos[i0].lshell = !values.d_nan
            infos[i0].fptmlt = !values.d_nan
            infos[i0].fptmlat = !values.d_nan
        endif else begin
            get_data, pre0+'mlt', data = dat
            infos[i0].mlt = sinterpol(dat.y, dat.x, deut)
            get_data, pre0+'lshell', data = dat
            infos[i0].lshell = sinterpol(dat.y, dat.x, deut)
            get_data, pre0+'fpt_mlt', data = dat
            infos[i0].fptmlt = sinterpol(dat.y, dat.x, deut)
            get_data, pre0+'fpt_mlat', data = dat
            infos[i0].fptmlat = sinterpol(dat.y, dat.x, deut)
        endelse
    ...
    vars = 'psbl_de_info'
    tplot_save, vars, filename = infofn
