...
pro _2015_0218_02_load_edot0_mgse, event_info, varname=var, time_var=time_var
    ...
    if ~cdf_has_var(var, filename=data_file) then begin
        ...
        b0_gsm_var = prefix+'b_gsm'
        _2015_0218_02_load_b_gsm, event_info, varname=b0_gsm_var, time_var=time_var
...
pro _2015_0218_02_load_hope_info, event_info
    ...
    event_info['o_num_dens_partial'] = mean(o_dens,/nan)
    event_info['p_num_dens_partial'] = mean(p_dens,/nan)
    ...
    ; Use emfisis density.
    event_info['num_dens'] = event_info['emfisis_num_dens']
    avg_num_dens = event_info['num_dens']
    ...
    event_info['avg_ion_mass'] = avg_mass
    ...
    event_info['omega'] = f_g0*avg_bmag*avg_mass*2*!dpi
    event_info['omega_p'] = f_g0*avg_bmag*p_mass*2*!dpi
    ...
    e_temp = mean(get_var_data(prefix+'e_temp', in=time_range))
    event_info['t_p'] = p_temp
    event_info['t_o'] = o_temp
    event_info['t_e'] = e_temp
    ...
    event_info['p_mag'] = p_b
    coef = 1.6e-19*1e9*1e6
    p_p = mean(get_var_data(prefix+'p_temp', in=time_range)*$
        get_var_data(prefix+'p_density', in=time_range))*coef
    p_o = mean(get_var_data(prefix+'o_temp', in=time_range)*$
        get_var_data(prefix+'o_density', in=time_range))*coef
    p_e = mean(get_var_data(prefix+'e_temp', in=time_range)*$
        get_var_data(prefix+'e_density', in=time_range))*coef
    event_info['p_p'] = p_p
    event_info['p_o'] = p_o
    event_info['p_e'] = p_e
    event_info['beta_p'] = p_p/p_b
    event_info['beta_o'] = p_o/p_b
    event_info['beta_e'] = p_e/p_b
    event_info['beta'] = (p_p+p_o+p_e)/p_b


    ; Ion acoustic gyroradius.
    tt = event_info['t_e']+event_info['t_p']+event_info['t_o']
    tt = event_info['t_e']
    mm = event_info['avg_ion_mass']
    ...
    event_info['r_gsm'] = r_gsm
    event_info['r_sm'] = cotran(r_gsm, the_time, 'gsm2sm')
