...
function fig_overview_omega_band_v01, test=test, event_info=event_info
    ...
    time_range = time_double(['2015-01-04/10:30','2015-01-04/14:00'])
...
;        o_pa_var = rbsp_read_pa_spec(time_range, probe=probe, species='o', errmsg=errmsg, energy_range=[50,5e4], update=update)
        ...
        ; Cyclotron freqs.
        fc_vars = list()
        foreach species, ['e','p'] do fc_vars.add, rbsp_read_gyro_freq(time_range, probe=probe, species=species, get_name=1)
...
;        fc_vars.add, var
        ...
        add_setting, theta_combo_var, smart=1, dictionary($
            ...
            'colors', sgcolor(['red','black','blue','green','purple']) )
...
;        endforeach
        ...
        plot_vars = [keo_var,theta_combo_var,dens_var,e_en_var,$
            e_spinfit_var,e_spec_combo]
        plot_vars = [keo_var,theta_combo_var,$
            e_spinfit_var,e_spec_combo]
        nvar = n_elements(plot_vars)
        fig_letters = letters(nvar)
        fac_labels = ['||',tex2str('perp')+','+['out','west']]
        fig_labels = fig_letters+') '+['Aurora','B elev','N!De','e- spec','E','E spec']
        fig_labels = fig_letters+') '+['Aurora','B elev','E','E spec']
        ypans = [1.2,1.2,0.8,1.2,1,1.8]
        ypans = [1.2,1,0.8,1.5]
        
        the_vars = [e_en_var,keo_var,e_spec_combo]
        options, the_vars, zcharsize=label_size
        ...
        abs_xchsz = tmp[0]
        abs_ychsz = tmp[1]
        nasi_time = n_elements(ob_times)
        asi_xpad = 1
        ypans = [(pansize[0]+(margins[0]-2)*abs_xchsz-(nasi_time-1)*asi_xpad*abs_xchsz)/nasi_time,ypans]
        ypad = [1,fltarr(nvar-1)+0.4]
        all_poss = panel_pos(plot_file, ypans=ypans, fig_size=fig_size, $
            nypan=nvar+1, pansize=pansize, panid=panid, margins=margins, ypad=ypad)
        poss = all_poss[*,1:*]
        ...
        tplot_options, 'tickinterval', 20*60
...
;        xyouts, tx,ty,msg, normal=1;, color=sgcolor('white')
        ...
        if count ne 0 then begin
            ...
            msg = 'Pulsating aurora'
            p_color = sgcolor('black')            
            p_times = time_double('2015-01-04/'+['11:45','11:55'])
            p_mlats = [62.8,62.5]
            tx = mean(p_times)
            ty = yrange[0]
            tmp = convert_coord(tx,ty,data=1, to_normal=1)
            tx0 = tmp[0]
            ty0 = tmp[1]+ychsz*1.5
            foreach p_time, p_times, pid do begin
                tx = p_time
                ty = p_mlats[pid]
                tmp = convert_coord(tx,ty,data=1, to_normal=1)
                tx1 = tmp[0]
                ty1 = tmp[1]
                plots, [tx0,tx1],[ty0,ty1], normal=1, color=p_color
            endforeach
            xyouts, tx0,ty0-ychsz*0.8,msg, normal=1, alignment=0.5, charsize=label_size
            
            p_times = time_double('2015-01-04/'+['12:55','13:10'])
            p_mlats = [60.7,60.5]
            tx = mean(p_times)
            ty = yrange[0]
            tmp = convert_coord(tx,ty,data=1, to_normal=1)
    ...
    ;---ASI.        
        ...
        asi_pos = all_poss[*,0]
        asi_pos[0] = 2*xchsz
        asi_poss = sgcalcpos(1,nasi_time,position=asi_pos, xpad=asi_xpad)
        ;for ii=0,nasi_time-1 do plot, [0,1],[0,1],nodata=1,noerase=1,position=asi_poss[*,ii]
        cbpos = asi_poss[*,nasi_time-1]
        cbpos[0] = cbpos[2]+xchsz*0.8
        cbpos[2] = cbpos[0]+xchsz*0.8
        asi_cbpos = cbpos
    ...
    ;---ASI image.
        ...
        asi_mlt_images = fltarr([nasi_time,image_size])
        for ii=0,image_size[0]-1 do for jj=0,image_size[1]-1 do asi_mlt_images[*,ii,jj] = interpol(mlt_images[*,ii,jj],times, ob_times)
        index = where(finite(asi_mlt_images,nan=1), count)
        if count ne 0 then asi_mlt_images[index] = 0
        if asi_zlog eq 1 then begin
            asi_zzs = bytscl(alog10(asi_mlt_images), min=asi_log_zrange[0],max=asi_log_zrange[1], top=top_color)
        endif else begin
            asi_zzs = bytscl((asi_mlt_images), min=asi_zrange[0],max=asi_zrange[1], top=top_color)
        endelse
        ...
        ; ylabels.
        ytick_pos = (-1)*!dpi
        if mlt_type eq 'post_midn' then begin
            ytick_pos = 0
        endif else if mlt_type eq 'pre_midn' then begin
            ytick_pos = !dpi
        endif
        yrange = mlat_range
        ystep = 5
        ...
        ob_mlts = [1.9,2.2,2.5,2.9]
        ob_mlats = [67,64,67,64]
        foreach time, ob_times, tid do begin
            tpos = asi_poss[*,tid]

            ; Draw image.
            zzs = reform(asi_zzs[tid,*,*])
            ...
            if mlt_type eq 'pre_midn' then begin
                ...
                plots, [0,1], [0,0], color=sgcolor('silver')
                plots, [0,0], [0,-1], color=sgcolor('silver')
                plots, [1,1], [0,-1], color=sgcolor('silver')
                plots, [0,1], [-1,-1], color=sgcolor('silver')
            endif else if mlt_type eq 'night' then begin
                xrange = [-1,1]
                yrange = [-1,0]
                plot, xrange, yrange, nodata=1, noerase=1, $
            ...
            ; add label.
            rr = 1-(ob_mlats[tid]-min_mlat)/(90-min_mlat)
            tt = (ob_mlts[tid]*15-90)*constant('rad')
            tx = rr*cos(tt)
            ty = rr*sin(tt)
            tmp = convert_coord(tx,ty, /data, /to_normal)
            tx = tmp[0]
            ty = tmp[1]
            msg = 'Omega!Cband'
            xyouts, tx,ty+ychsz*2.2, msg, normal=1, alignment=0.5, charsize=label_size
            plots, tx+[0,0],ty+[0,1]*ychsz, normal=1
            
            tx = tpos[0]+xchsz*0.5
            ty = tpos[1]+ychsz*0.2
            msg = asi_letter+'-'+string(tid+1,format='(I0)')+') '+time_string(time,tformat='hh:mm:ss')+' UT'
            xyouts, tx,ty, msg, normal=1, alignment=0, charsize=label_size
...
print, fig_overview_omega_band_v01(test=0, event_info=omega_band_event)
end