...
test = 0
    ...
    file = join_path([project.plot_dir, 'fig_v2d_on_xy_plane5.pdf'])
...
;---Plot settings.
    ...
    feint_colors = ['CD6666'x,'336633'x,'6666CD'x,'663366'x]
    feint_colors = ['CD8B8B'x,'A4CDA4'x,'8B8BCD'x,'CDA4CD'x]
;    ; The vertex.
;    foreach event, events do begin
;        direction = (strsplit(event.region,'%',/extract))[1]
;        color = (feint_colors[where(directions eq direction)])[0]
;        foreach triad, event.triad_list do begin
;            rsms = triad.r_sms
;            center_r_sm = triad.center_r_sm
;            xs = rsms[*,0]
;            ys = rsms[*,1]
;            for ii=0,2 do plots, [xs[ii],center_r_sm[0]], [ys[ii],center_r_sm[1]], color=color, thick=0.25
...
;    endforeach
    ...
    ; Add arrow.
    tx = tpos[0]+xchsz*1
    ty_label = tpos[1]+ychsz*3.2
    foreach key, directions, key_id do begin
        ty = ty_label-ychsz*0.9*key_id
        case key of
            'earthward': msg = 'radial.in'
            'outward': msg = 'radial.out'
            else: msg = key
        endcase
        xyouts, tx,ty,/normal, msg, color=colors[key_id]
    endforeach
...
;---Plot v_2d arrows.
    foreach event, events, event_id do begin
        ...
        foreach triad, event.triad_list do begin
            center_r_sm = triad.center_r_sm
            plots, center_r_sm[0],center_r_sm[1], psym=fill_psym, $
                symsize=fill_symsize, color=color
            ...
            x1 = x0+v_hat[0]*scale
            y1 = y0+v_hat[1]*scale
            arrow, x0,y0,x1,y1,/data, solid=arrow_solid, hsize=hsize, color=color, thick=2
        endforeach
    endforeach
