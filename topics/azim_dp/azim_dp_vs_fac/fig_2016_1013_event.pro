;+
; Generate the summary plot for the 2016-10-13 event.
; fig_2016_1013_2016_1013_v01.pdf
;-
...
;    the_line = dictionary($
;        'times', time_double(['2016-10-13/12:18','2016-10-13/12:50']), $
;        'omega_linfit', -4.6, $
;        'mlts', [0,10] )
    the_line_style = 2
...
;---Add line.
    ...
    foreach panel_id, [1,2,3] do begin
        ...
        plot, xrange, yrange, /nodata, /noerase, $
            xstyle=5, ystyle=5, position=tpos
;        omega = the_line.omega_linfit
;        txs = the_line.times
;        tys = the_line.mlts[0]-[0,(txs[1]-txs[0])*omega/60./15]
;        oplot, txs,tys, linestyle=the_line_style, color=the_line_color
        omega1 = the_start.omega1
        omega2 = the_start.omega2
;        omega = the_start.omega_linfit
;        if panel_id eq 1 then begin
;            tx = tpos[0]+xchsz*0.5
;            ty = tpos[3]-ychsz*0.8
;            msg = tex2str('omega')+'= '+strtrim(string(omega,format='(F5.1)'),2)+' deg/min'
;            xyouts, tx,ty,/normal, msg, charsize=label_size
;        endif
