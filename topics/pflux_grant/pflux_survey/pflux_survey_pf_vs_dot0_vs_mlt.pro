;+
; Study the different between S and dot0.
;-
...
;---Pick out times when Edot0 are not NaNs.
    ...
    foreach probe, probes do begin
        prefix = 'rbsp'+probe+'_'
        get_data, prefix+'pfdot0_fac_norm_interp', times, dedot0
        get_data, prefix+'pf_fac_norm_interp', times, de
...
;        dedot0 = dedot0[0:*:step,*]
        ...
        fig_xsize = 8
        fig_ysize = 3
        plot_file = join_path([project.plot_dir,'pflux_survey','figures','fig_pf_vs_dot0_rbsp'+probe+'_vs_mlt.pdf'])
        ...
        for ii=0,ndim-1 do begin
            ...
            txs = de[*,ii]
            tys = dedot0[*,ii]
            xtitle = short_name+' ('+unit+')'
            ytitle = short_name+'!Ddot0!N ('+unit+')'
            if ii ne 0 then ytitle = ' '
            if ii ne 0 then ytickformat='(A1)' else ytickformat=''

            plot, range,range, /iso, $
                xstyle=1, xrange=range, xlog=log, xtitle=xtitle, xticklen=ticklen, xtickv=xtickv, xticks=xticks, xminor=xminor, $
                ystyle=1, yrange=range, ylog=log, ytitle=ytitle, yticklen=ticklen, ytickv=ytickv, yticks=yticks, yminor=yminor, $
                position=tpos, /noerase, ytickformat=ytickformat, /nodata

            plots, range, [0,0], linestyle=1, color=sgcolor('silver')
            plots, [0,0], range, linestyle=1, color=sgcolor('silver')
            plots, range, range, linestyle=1, color=sgcolor('silver')
