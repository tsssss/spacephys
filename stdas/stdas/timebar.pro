PRO timebar,t1,color=color0,linestyle=linestyle,thick=thick,verbose=verbose,$
...
@tplot_com
  ...
  nt = n_elements(t)
  if not keyword_set(color0) then begin
    if !p.background eq 0 then color = !d.n_colors-1 else color = 0
  endif
  if n_elements(color0) ne nt then color = make_array(nt,value=color0) else color = color0
