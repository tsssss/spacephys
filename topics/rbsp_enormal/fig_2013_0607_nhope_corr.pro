...
butr = time_double(['2013-06-07/04:56','2013-06-07/05:01'])
butr = time_double(['2013-06-07/04:55','2013-06-07/05:12'])
probes = ['a','b']
...
get_data, 'rbspa_n', uts, avsc
idx = where(finite(avsc))
avsc = interpol(avsc[idx],uts[idx],uts)
avsc = alog10(avsc)
store_data, 'rbspa_'+tvar, uts, avsc

get_data, 'rbspb_n', uts, bvsc
idx = where(finite(bvsc))
bvsc = interpol(bvsc[idx],uts[idx],uts)
bvsc = alog10(bvsc)
store_data, 'rbspb_'+tvar, uts, bvsc
...
ofn = shomedir()+'/fig_nhope_correlation.pdf'
;ofn = 0
...
plot, txs, tys, xstyle = 1, ystyle = 9, yrange = yr, ylog = ylog, $
    ytitle = 'RBSP-A!Cn (cc)', position = tpos, $
    xticks = 3, xminor = 6, xticklen = 1, xgridstyle = 1, $
    xtitle = 'Second from '+time_string(uts[idx[0]])
axis, max(txs), /yaxis, yrange = yr-dvsc, ytitle = 'RBSP-B!Cn (cc)'
