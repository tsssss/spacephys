...
pro rbsp_mgse2gse, tname, wgse, newname = newname, inverse = inverse, $
    probe = probe, no_spice_load = no_spice_load, dt = dt
    ...
    if n_elements(wgse) ne 0 then begin
        ...
        if ~keyword_set(no_spice_load) then begin
            rbsp_efw_init
            rbsp_load_spice_kernels, probe = probe
        endif
        t0 = dat.x
        nrec = n_elements(t0)
        tts = (n_elements(dt) eq 0)? t0: smkarthm(t0[0], t0[nrec-1], dt, 'dx')
        ntt = n_elements(tts)
        wgse = dblarr(ntt,3)
        for i = 0, ntt-1 do begin
            tstr = time_string(tts[i], tformat = 'YYYY-MM-DDThh:mm:ss.ffffff')
            cspice_str2et, tstr, et
            cspice_pxform, 'RBSP'+prob+'_SCIENCE', 'GSE', et, pxform
            wgse[i,*] = pxform[2,*]
        endfor
        wx = interpol(wgse[*,0], tts, t0)
        wy = interpol(wgse[*,1], tts, t0)
        wz = interpol(wgse[*,2], tts, t0)
        wgse = [[wx],[wy],[wz]]
    endelse 
