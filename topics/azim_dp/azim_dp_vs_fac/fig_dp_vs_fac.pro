...
;---Calculate fig size.
    ...
    margins = [9,4,6,1]
    ; panel sizes.
    ypans = [0.6,1,0.8,1,1]*0.9
    nypanel = n_elements(ypans)
    ypads = 0.4+dblarr(nypanel-1)
    fig_labels = $
        ['AE','Scaled '+tex2str('theta'),'J vert','J up','J down']
    panel_vars = ['ae','scaled_theta','j_vert','j_up','j_down']
...
;---J color bar.
    tpos = all_poss[*,nxpanel-1,nypanel-1]
    cb_width = xchsz*0.7
    j_cbpos = tpos[[2,1,2,3]]+[1,0,2,0]*cb_width
    j_cbpos[3] = all_poss[3,nxpanel-1,2]
...
;---Adopted from fig_ewogram_of_dp_and_up_down_current.pro.
    ...
    ;---Add snapshot times.
        if ~event.haskey('snapshot_times') then continue
        snapshot_times = event.snapshot_times
        if ~event.haskey('snapshot_label') then snapshot_label = '' else snapshot_label = event.snapshot_label
        foreach snapshot_time, snapshot_times, time_id do begin
            tx = snapshot_time
            ty = tpos[1]
            tmp = convert_coord(tx,ty, /data, /to_normal)
            txs = tmp[0]+[0,0]
            tys = ty+[-0.15,0.15]*ychsz
            plots, txs,tys,/normal, thick=thick*2
            xyouts, txs[0],tys[0]-ychsz*0.7,/normal, alignment=0.5, $
                snapshot_label+string(time_id+1,format='(I0)'), charsize=label_size
        endforeach
    endfor
