;+
; Load omni parameters (AE, Dst, solar wind parameters)
;-

function ml_omni_load_param, input_time_range, id=datatype, $
...
dates = string(make_bins([1996,2008],1),format='(I04)')
dates = string(make_bins([1996,2011],1),format='(I04)')
;dates = string(make_bins([2011,2019],1),format='(I04)')
foreach date, dates do begin
    foreach probe, rbsp_probes() do begin
    files = ml_omni_load_param(date)
    endforeach
endforeach
stop
