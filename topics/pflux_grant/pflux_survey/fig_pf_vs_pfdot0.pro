...
test = 0
    ...
    probes = ['a','b']
    plot_file = join_path([project.plot_dir,'figures','fig_pf_vs_pfdot0_vs_mlt.pdf'])
...
;    data_file2 = join_path([tmp.data_dir,'pflux_survey_pf_vs_pfdot0.tplot'])
;    if file_test(data_file2) eq 1 then tplot_restore, filename=data_file2
    ...
    ;---Load pflux.
        foreach pf_var, pf_vars do begin
            pf_var = prefix+pf_var+'_fac_norm'
            pf_save_var = pf_var+'_interp'
            save_vars.add, pf_save_var
            if ~check_if_update(pf_save_var) then continue
            file_updated = 1
            ...
            for day_id=0, nday-1 do begin
                ...
                index = where_pro(common_times,'[)', time_range)
                pf_data[index,*] = (get_var_data(pf_var))[0:*:de_step,*]
            endfor
            store_data, pf_save_var, common_times, pf_data
            short_name = (pf_var eq 'pf')? 'S': 'S!SUdot0!R!N'
            add_setting, pf_save_var, /smart, {$
                display_type: 'vector', $
                unit: 'mW/m!U2!N', $
                short_name: short_name, $
                coord: '', $
...
;---Remove data when bw_ratio does not meet criteria.
    ...
    foreach probe, probes do begin
        ...
        pfdot0_fac = get_var_data(prefix+'pfdot0_fac_norm_interp')
        pf_fac = get_var_data(prefix+'pf_fac_norm_interp')
        r_gsm = get_var_data(prefix+'r_gsm')
        pf_range.add, minmax(pf_fac)
        pf_range.add, minmax(pfdot0_fac)

        if ~check_if_update(prefix+'pf_fac') then continue
        index = where(dedot0_flag eq 0)
        times = common_times[index]
        pfdot0_fac = pfdot0_fac[index,*]
        pf_fac = pf_fac[index,*]
        r_sm = cotran(r_gsm[index,*], common_times, 'gsm2sm')
        ; Convert "earthward" to "parallel"
        index = where(r_sm[*,2] le 0)
        pf_fac[index,0] *= -1
        pfdot0_fac[index,0] *= -1
        mlt = pseudo_mlt(r_sm)
        store_data, prefix+'pf_fac', times, pf_fac
        store_data, prefix+'pfdot0_fac', times, pfdot0_fac
        store_data, prefix+'mlt', times, mlt
    endforeach
    pf_range = minmax(pf_range.toarray())
    de_tick_step = 200.
;    pf_range = [-1,1]*ceil(max(abs(pf_range))/de_tick_step)*de_tick_step
;    de_tickv = make_bins(pf_range,de_tick_step*2,/inner)
    pf_range = [-1,1]*440
    de_tickv = make_bins(pf_range,de_tick_step,/inner)
...
;---The plot.
    ...
    poss = sgcalcpos(1,npanel, xchsz=xchsz, ychsz=ychsz, xpad=xpad, margins=margins)
...
;---Panel settings.
    psym = 6
    symsize = 0.2
    xtitle = short_name+'!D0!N ('+pf_unit+')'
    ...
    for panel_id=0,ndim-1 do begin
        ...
        if panel_id ne 0 then ytitle = ' ' else ytitle = short_name+'!Ddot0!N ('+pf_unit+')'
        ...
        plot, pf_range,pf_range, /iso, $
            xstyle=1, xrange=pf_range, xlog=de_log, xtitle=xtitle, xticklen=de_ticklen, xtickv=de_tickv, xticks=de_ticks, xminor=de_minor, $
            ystyle=1, yrange=pf_range, ylog=de_log, ytitle=ytitle, yticklen=de_ticklen, ytickv=de_tickv, yticks=de_ticks, yminor=de_minor, $
            position=tpos, /noerase, ytickformat=ytickformat, /nodata
        plots, pf_range,pf_range, linestyle=1, color=line_color
        plots, [0,0],pf_range, linestyle=1, color=line_color
        plots, pf_range,[0,0], linestyle=1, color=line_color
        ...
        foreach mlt_center, mlt_centers, mlt_id do begin
            ...
            foreach probe, probes do begin
                ...
                prefix = 'rbsp'+probe+'_'
                xdata = (get_var_data(prefix+'pf_fac'))[*,panel_id]
                ydata = (get_var_data(prefix+'pfdot0_fac'))[*,panel_id]
                mlts = get_var_data(prefix+'mlt')

                index = where_pro(mlts,'[)', mlt_vertices[mlt_id,*], count=count)
                ...
                plots, xxs,yys, color=colors[mlt_id], psym=psyms[panel_id], symsize=symsize
            endforeach
            slope = mean(slope.toarray())
            slopes[mlt_id] = slope
            ;print, mlt_centers[mlt_id], slope
            ;oplot, pf_range,pf_range*slope, linestyle=1, color=colors[mlt_id]
        endforeach
