...
pro wygant_asi_diff_keogram, tr
    ...
    ; diff keogram and ewogram.
    t0 = t0[1:nrec-1]
    keo = keo[1:nrec-1,*]-keo[0:nrec-2,*]
    ewo = ewo[1:nrec-1,*]-ewo[0:nrec-2,*]
    var = 'thg_atha_dkeo'
    store_data, var, t0, keo, lats
    options, var, 'zrange', minmax(keo)*0.5
    var = 'thg_atha_dewo'
    store_data, var, t0, ewo, lons
    options, var, 'zrange', minmax(ewo)*0.5
    
    vars = 'thg_atha_'+['keo','ewo','dkeo','dewo']
    options, vars, 'spec', 1
...
;    options, var, 'psym', 0
    ...
    var = 'comb_dkeo'
    store_data, var, data = ['thg_atha_dkeo','rbspb_mlat']
    options, var, 'yrange', range_mlat
    options, var, 'ytitle', 'mlat (deg)'
    
    var = 'comb_dewo'
    store_data, var, data = ['thg_atha_dewo','rbspb_mlon']
    options, var, 'yrange', range_mlon
    options, var, 'ytitle', 'mlon (deg)'
    
    var = 'rbspb_pf_fac_mat'
    options, var, 'labels', ['Sb total','Sp','Sv']
    
    tplot_options, 'constant', 0
    tplot_options, 'labflag', -1
        
    vars = ['comb_keo','comb_dkeo','comb_ewo','comb_dewo','rbspb_pf_fac_mat','rbspb_atha_count']
    ...
    ofn = shomedir()+'/tmp/2013_0501_diff_keo_ewo_'+tmp+'.pdf'
    sgopen, ofn, xsize = 5, ysize = 7, /inch
    sgindexcolor, 43
    tplot, vars, trange = tr, var_label = labs, title = titl, /noerase
    sgclose
end

tr = time_double(['2013-05-01/07:30','2013-05-01/07:50'])
;tr = time_double(['2013-05-01/05:00','2013-05-01/10:00'])
;tr = time_double(['2013-05-01/06:55','2013-05-01/08:15'])
;tr = time_double(['2013-05-01/05:30','2013-05-01/06:10'])

wygant_asi_diff_keogram, tr
end