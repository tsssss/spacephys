...
function fig_2015_0312_0900_dmsp_overview_v01, plot_file, event_info=event_info, test=test
...
;            xyouts, tx,ty,normal=1, msg, charsize=label_size, color=sgcolor('red')
    ...
    xyouts, tx1+xchsz*0.5, ty1-ychsz*0.2, normal=1, 'DMSP '+strupcase(dmsp_probe), charsize=sc_label_size, color=dmsp_color
...
;---Auroral snapshots.
    ...
    aurora_info.ssusi = dictionary($
        'msg', 'c-2) South | '+ssusi_wavelength+'!C'+strupcase(dmsp_probe)+' | '+strjoin(time_string(ssusi_time_range,tformat='hh:mm'),'-')+' UT', $
        'hemisphere', 'south', $
        'position', poss[*,1,1], $
        'zzs', ssusi_zzs, $
        'ct', ct_ssusi )
    ...
    foreach the_info, aurora_info do begin
        ...
        xyouts, tx1+xchsz*0.5, ty1+ychsz*0.2, msg, normal=1, alignment=0.5
    endforeach
...
test = 0
print, fig_2015_0312_0900_dmsp_overview_v01(event_info=event_info, test=test)
end
