...
test = 0
    ...
    ofn = join_path([root_dir,'case2_fig_field_line_trace.pdf'])
    ...
    foreach line, line_info, ii do begin
        ...
        for jj=0,ndata-1 do begin
            ...
            geopack_t89, par, fline[jj,0],fline[jj,1],fline[jj,2], tbx,tby,tbz
    ...
    tx = tpos[2]-xchsz*0.5
    ty = tpos[3]-ychsz*1
    msg = 'Model (T89) magnetic field lines at local midnight'
    xyouts, tx,ty,alignment=1, /normal, msg, charsize=label_size

    yspace = 0.9
    label_size = 0.9
    msg = 'Downward current field lines: ['+strjoin(string(down_current.lat_range,'(I0)'),',')+'] deg'
    xyouts, tx,ty-ychsz*1*yspace,alignment=1, /normal, msg, color=down_current.color, charsize=label_size
    msg = 'Upward current field lines: ['+strjoin(string(up_current.lat_range,'(I0)'),',')+'] deg'
    xyouts, tx,ty-ychsz*2*yspace,alignment=1, /normal, msg, color=up_current.color, charsize=label_size
    msg = 'Proxy for high-beta plasma, |B|<'+string(min_bmag,format='(I0)')+' nT'
    xyouts, tx,ty-ychsz*3*yspace,alignment=1, /normal, msg, color=min_bmag_color, charsize=label_size
