...
pro azim_prop_gen_storm_ewogram, time_range, $
...
;---Preparation.
    ...
    if n_elements(dis_range) ne 2 then dis_range = [4,13]
...
;---Generate the plot.
    ...
    file = join_path([homedir(),'fig_azim_prop_'+time_string(time_range[0],tformat='YYYY_MMDD')+'_storm_overview.pdf'])
    ...
    labels = letters(npanel)+'. '+['Dst','AE','UT-MLT','UT-R']
...
;---The ewogram.
    ...
    zrange = tilt_range
    xticklen = -0.02
    yticklen = xticklen*0.25*(tpos[3]-tpos[1])
    yminor = 3
...
;---The keogram.
    ...
    foreach mission_probe, mission_probes do begin
        ...
        goes_shift = 0.25
        goes_dis = 6.6
        if mission_probe eq 'g13' then yys -= goes_shift
        if mission_probe eq 'g15' then yys += goes_shift

        tx = tpos[0]
        tmp = convert_coord(0,goes_dis+goes_shift, /data, /to_normal)
        y1 = tmp[1]
        tmp = convert_coord(0,goes_dis-goes_shift, /data, /to_normal)
        y2 = tmp[1]
        plots, tx-[0,0.25]*xchsz, y1+[0,0], /normal
        plots, tx-[0,0.25]*xchsz, y2+[0,0], /normal
        plots, tx-0.25*xchsz+[0,0], [y1,y2], /normal
        tmp = convert_coord(0,goes_dis, /data, /to_normal)
        ty = tmp[1]
        plots, tx-[0.25,0.5]*xchsz, ty+[0,0], /normal
        xyouts, tx-xchsz*0.75, ty-ychsz*0.35*label_size, /normal, sgnum2str(goes_dis), charsize=label_size, alignment=1

        msg = 'GOES shifted in R!Cto avoid overlap'
        tmp = convert_coord(0,goes_dis, /data, /to_normal)
        ty = tmp[1]
        xyouts, tx-xchsz*2.5, ty, /normal, msg, charsize=label_size, alignment=1
...
;    storm_list.add, time_double(['2017-03-27/00:34:56','2017-03-28/17:36:00'])
    ...
    foreach time_range, storm_list do azim_prop_gen_storm_ewogram, time_range
