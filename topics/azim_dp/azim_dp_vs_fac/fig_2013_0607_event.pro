;+
; Generate the summary plot for the 2013-06-07 event.
;-
...
;---Settings for the start location and slopes (omega).
    ...
    the_start = dictionary($
        'times', time_double(['2013-06-07/02:57','2013-06-07/04:40','2013-06-07/05:40']), $
        'mlts', [-3.5,0,1], $
        'omega2', [8,2.0,3], $
        'omega1', [-3.5,-2,-3] )
...
;---Panel e. KEOgram.
    ...
    ; Add snapshot times.
    ty = tpos[1]
    foreach time, snapshot_times, time_id do begin
        tx = time
        tmp = convert_coord(tx,ty, /data, /to_normal)
        txs = tmp[0]+[0,0]
        tys = ty+[-0.3,0]*ychsz
        plots, txs,tys,/normal, thick=thick*2
        xyouts, txs[0],tys[0]-ychsz*0.8,/normal, alignment=0.5, $
