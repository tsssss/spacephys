;+
; Calculate azimuthal angle of a vector (B, E, etc).
; vec_var.
; coord=.
;-
function lets_calc_vec_azim, vec_var, coord=coord_need, $
    var_info=var_info, $
    ...
    stop
    vec_azim = atan(the_vec[*,1],the_vec[*,0])*constant('deg')

    store_data, var_info, times, vec_azim
    add_setting, var_info, smart=1, dictionary($
        'display_type', 'scalar', $
        'short_name', short_name+'!D'+tex2str('phi'), $
        'unit', 'deg' )
