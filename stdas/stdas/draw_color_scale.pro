...
if keyword_set(zhorizontal) then begin
    ...
    x = findgen(nxpix)*(!y.crange[1]-!y.crange[0])/(nxpix-1)+!y.crange[0]
    if keyword_set(log) then x = 10.^x
    x = bytescale(x,bottom=brange[0],top=brange[1],range=range,log=log)
    image = x # replicate(1b,npy)
endif

; Sheng.
sg_colormode, decomposed=color_mode, depth=depth
device, decomposed=0
...
if pos[2]-pos[0] ne 0 and pos[3]-pos[1] ne 0 then begin
  ...
  if keyword_set(yticks) then begin
        if keyword_set(zhorizontal) then begin
            axis, xaxis=1, xstyle=1, xrange=range, xlog=log, xtitle=title, charsize=charsize,xticks=yticks,xtickname=ytickname,_extra=ex
        endif else begin
    ...
    if not keyword_set(title) then begin
      ...
      endelse
        ...
        if keyword_set(zhorizontal) then begin
            axis, xaxis=1, xstyle=1, xrange=range, xlog=log, xtitle=title, charsize=charsize,xticks=1,_extra=ex
        endif else begin
            axis, yaxis = 1, ystyle = 1, yrange = range, ylog = log, ytitle = title, charsize = charsize, yticks = 1,_extra=ex
        endelse
  endif else begin
        if keyword_set(zhorizontal) then begin
            axis, xaxis=1, xstyle=1, xrange=range, xlog=log, xtitle=title, charsize=charsize,_extra=ex
        endif else begin
