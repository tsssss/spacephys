...
;-
pro vars_on_asi_2013_0501, asivar, asivars = asivars, pltvars = pltvars, $
...
; restore, v_exb, pf.
root1 = shomedir()+'/Google Drive/works/works/wygant/rbsp_thg/pflux_thm_asi'
restore, filename = root1+$
    '/rbspb_2013_0501_pflux_scott/RBSPb_ExB_vels_FAC_11_sec_ave_fields_2013-05-01.dat'
...
root2 = shomedir()+'/Google Drive/works'
fn = root2+'/data/rbsp_thg/rbsp_efw_fld_2013_0501_04.tplot'
...
uts = time_double(['2013-05-01/07:38:06','2013-05-01/07:38:06'])
uts = smkarthm(uts[0],uts[1],3,'dx')
sg = 'ps'
tr = time_double(['2013-05-01/07:25','2013-05-01/07:55'])
pltvars = ['rbspb_'+['de_fac','db_fac','pf_fac_mat','atha_count']]


perp = '!9'+string(94b)+'!X'
labfac = ['||',perp+',West',perp+',North']

options, 'rbspb_'+['de_fac','db_fac','pf_fac_mat'], 'labels', labfac

tvar = 'rbspb_atha_count'
options, tvar, 'ylog', 0
options, tvar, 'ytitle', 'ATHA count!C@RBSP-B Fpt'
...
for i = 0, n_elements(uts)-1 do begin
    ...
    vars_on_asi_2013_0501, asivar, asivars = asivars, pltvars = pltvars, vecvars = vecvars, trange = tr, sg = sg, minlat = minlat
endfor
cdf_close, moid
...
pos0 = sgcalcpos(2, position = [0.15,0.1,0.8,0.95], ypad = 10)
vars = 'rbspb_fpt_lonlat_thg_'+sites+'_cr'
nvar = n_elements(vars)
ylim, vars, 1,100, 1
options, vars, 'ystyle', 1
ofn = shomedir()+'/thg_photon_count_2013_0501_log_scale.pdf'
sgopen, ofn, xsize = 5, ysize = 6, /inch
tr = time_double(['2013-05-01/07:14','2013-05-01/08:14'])
titl = 'Photon count, '+stodate(tr[0],'%H:%M')+' - '+stodate(tr[1],'%H:%M')
poss = sgcalcpos(nvar, position = posu)
tplot, vars, trange = tr, position = sgcalcpos(nvar, position = pos0[*,0]), /noerase, title = titl
tr = time_double(['2013-05-01/07:30','2013-05-01/07:50'])
titl = 'Photon count, '+stodate(tr[0],'%H:%M')+' - '+stodate(tr[1],'%H:%M')
tplot, vars, trange = tr, position = sgcalcpos(nvar, position = pos0[*,1]), /noerase, title = titl
sgclose

vars = 'rbspb_fpt_lonlat_thg_'+['atha','tpas']+'_cr'
nvar = n_elements(vars)
ylim, vars, 0,100, 0
options, vars, 'ystyle', 1
ofn = shomedir()+'/thg_photon_count_2013_0501_linear_scale.pdf'
sgopen, ofn, xsize = 5, ysize = 6, /inch
tr = time_double(['2013-05-01/07:14','2013-05-01/08:14'])
titl = 'Photon count, '+stodate(tr[0],'%H:%M')+' - '+stodate(tr[1],'%H:%M')
poss = sgcalcpos(nvar, position = posu)
