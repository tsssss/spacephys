...
;+
; Remove moon for this event.
;-

function themis_asf_plot_raw_image, image, zrange=zrange, ct=ct, position=tpos

    if n_elements(zrange) eq 0 then zrange = [-1,1]*1.5e4
    zz = bytscl(reform(image), min=zrange[0], max=zrange[1], top=top_color)
    if n_elements(ct) eq 0 then ct = 70
    
    if n_elements(tpos) eq 0 then begin
        plot_file = 0
        tpos = panel_pos(plot_file, pansize=[6,6], margins=[8,4,2,1], fig_size=fig_size)
        sgopen, plot_file, size=fig_size, xchsz=xchsz, ychsz=ychsz
    endif
    sgtv, zz, ct=ct, position=tpos
    image_size = size(zz, dimensions=1)
    ...
    plot, xrange, yrange, $
        ...
        xrange=xrange, xstyle=1, xtitle=xtitle, xtickv=xtickv, xticks=xticks, xminor=xminor, $
        yrange=yrange, ystyle=1, ytitle=ytitle, ytickv=ytickv, yticks=yticks, yminor=yminor, $
        xticklen=xticklen, yticklen=yticklen
    line_color = sgcolor('silver')
    foreach xx, xtickv do plots, xx+[0,0], yrange, linestyle=1, color=line_color
    foreach yy, ytickv do plots, xrange, yy+[0,0], linestyle=1, color=line_color
    
    return, 1

end

function test_themis_asf_calc_background_v02, input_data, moon_angles, min_data=min_data, test=test
...
;    trial_offsets = [0,0.25,0.5,0.75]*value_step
    ...
    if keyword_set(test) then begin
        sgopen, 0, size=[12,4]
        plot, the_data, yrange=[100,7e4], xstyle=1, ylog=0
    endif
    
    ; Get 1st order estimate of the background.
    ; linear value bins are used to make it denser at larger values.
    ...
    index = where(the_data ge max_value, count)
    if count ne 0 then sample_indexs = [sample_indexs,index]
    
    
    sample_indexs = sort_uniq(sample_indexs)
    if keyword_set(test) then plots, sample_indexs, the_data[sample_indexs], psym=-1, symsize=0.5, color=sgcolor('red')
    
    
    index_range = [0,ndata-1]
    index_step = 600d/3
    time_indexs = sort_uniq([index_range,make_bins(index_range,index_step, inner=1)])
    if keyword_set(test) then plots, time_indexs, the_data[time_indexs], psym=-1, symsize=0.5, color=sgcolor('cyan')
    stop
...
time_range = time_double(['2015-01-04/08:00','2015-01-04/14:00'])
site = 'whit'
foreach site, ['fykn','fsim','atha','gill'] do begin
    var = themis_asf_read_mlt_image_rect(time_range, site=site, calibration_method='moon', min_elev=5)
    stop
endforeach
...
;;    moon_weight = exp((2.5-moon_angles)/2.5)*2+1
    ...
    plot_file = join_path([srootdir(),'test_themis_asf_calc_background_v02.pdf'])
    plot_file = 1
    ...
    oplot, the_counts
    plots, [0,ntime-1], 65535+[0,0], linestyle=1
    tx = tpos[0]+xchsz*0.5
    ty = tpos[3]-ychsz*1
    msg = label
    xyouts, tx,ty,msg, normal=1

;;    sample_indexs = test_themis_asf_calc_background_v02(the_counts, test=1, moon_angles)
tic
    bg2 = test_themis_asf_calc_background_v02(the_counts, test=0)
toc
