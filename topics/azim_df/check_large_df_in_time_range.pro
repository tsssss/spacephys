...
pro check_large_df_in_time_range, time_range, probes=probes, search_step=search_step

    time0 = time_double('2012-01-01')
    region_name = (mean(time_range) ge time0)? 'within_15Re': 'beyond_15Re'
    search_names = ['post','pre']
    project = azim_df_load_project()
    mlt_range = [-1,1]*9
    rxy_range = [4.,30]

    the_var = 'all_events'+region_name
    if check_if_update(the_var) then begin
        events = list()
        foreach search_name, search_names do begin
            file = join_path([project.data_dir,'azim_df_search_event_'+search_name+'_midn_'+region_name+'_search_'+search_step+'.txt'])
    ...
    df_list = list()
    df_str_list = list()
    events = get_var_data(the_var)
    foreach event, events do begin
        foreach df, event.df_list do begin
            if product(df.obs_time-time_range) ge 0 then continue
            df_str = df.probe+time_string(df.obs_time)
            if df_str_list.where(df_str) ne !null then continue
            df_str_list.add, df_str
            df_list.add, df
        endforeach
    endforeach
...
;time_range = time_double(['2008-03-28/08:00','2008-03-28/09:20'])
;time_range = time_double(['2009-04-13/02:30','2009-04-13/03:30'])
;time_range = time_double(['2014-12-26/00:30','2014-12-26/01:50'])
;time_range = time_double(['2016-06-15/04:00','2016-06-15/05:00'])
;time_range = time_double(['2017-03-31/02:40','2017-03-31/03:30'])
...
; Check theta above 0.
;time_range = time_double(['2016-11-13/12:01','2016-11-13/13:30'])
time_range = time_double(['2016-10-13/11:30','2016-10-13/14:00'])
time_range = time_double(['2007-11-20/17:01','2007-11-20/17:16'])+[-1,1]*60*20


search_step = 'df'
check_large_df_in_time_range, time_range, search_step=search_step
