...
pro read_themis_vbulk, utr0, probe=probe, newname=newname, $
...
;---prepare var names.
    pre0 = 'th'+prb+'_'
    var0s = pre0+['peir_time','peir_velocity_gsm']   ; 3 sec resolution.
...
;---Move data to a structure then to tplot.
    ...
    vgsm = sinterpol(dat.(1), dat.(0), ut0s)
    
    myvars = pre0+['vbulk_gsm']
    if not keyword_set(newname) then newname = 'vbulk_gsm'
    store_data, pre0+newname, ut0s, vgsm, $
        limits = {ytitle:'(km/s)', colors:[6,4,2], labels:'GSM V'+['x','y','z'], labflag:-1}
