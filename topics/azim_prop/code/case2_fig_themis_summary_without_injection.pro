...
test = 0
    ...
    ;---Plot.
        ...
        ofn = join_path([root_dir,'case2_fig_'+probe+'_summary_without_injection.pdf'])
        ...
        ypad = 0.4
        margins = [10,4,8,2]
        vars = prefix+['ele_en_spec','ion_en_spec','ele_n','b_gsm','e_gsm','pf_para','theta']
        nvar = n_elements(vars)
        labels = letters(nvar)+' .'+['Ele','Ion','Density','B GSM','E GSM','S!D||!N','Tilt']
        fig_xsize = pan_xsize+total(margins[[0,2]])*xchsz
        fig_ysize = pan_ysize*nvar+(ypad*(nvar-1)+total(margins[[1,3]]))*ychsz
