...
; vecvars: vectors plotted on aurora image. in [n,2], [i,0] is vector's origin, [i,1] is the vector.
; 
; run read_thm_asi_mosaic and rbsp_thg_prep_data first.
;-
pro vars_on_asi_2013_0414, asivar, asivars = asivars, pltvars = pltvars, $
...
;    tv, timg, asipos[0], asipos[1], /normal, /nan
    ...
    tplot, pos = varpos, vars, /noerase, trange = tr, title = ''
...
;    '/works/wygant/pflux_thm_asi/rbspb_2013_0414_pflux_scott/RBSPb_ExB_vels_FAC_11_sec_ave_fields_2013-05-01.dat'
...
fn = shomedir()+'/Google Drive/works/data/rbsp_thg/rbsp_efw_fld_2013_0414_04.tplot'
tplot_restore, filename = fn

var = 'rbsp*_pf_fac_mat'
...
vars = ['rbspa_de_fac','rbspa_db_fac']
options, vars, 'colors', [r,g,b]

; load count rate along s/c footpoint.
vars = ['rbspa_fpt_lonlat_thg_'+sites+'_cr']
vars = tnames(vars)
ylim, vars, 1, 100, 1

scle = 0.05     ; 1 mV/m ~ 10 deg.
scle = 1        ; for log field.
get_data, 'rbspa_de_fac', t0, dat
store_data, 'rbspa_de_vec', t0, dat[*,1:2], scle, limits = {colors:white, linestyle:0}
vecvars = [['rbspa_fpt_lonlat'],['rbspa_de_vec']]

asivar = 'asi'
asivars = tnames('rbspa_fpt_lonlat')
pltvars = ['rbspa_'+['de_fac','db_fac','vexb','pf_fac_mat','fpt_lonlat_thg_'+sites+'_cr']]
tr = sfmepoch(etr,'unix')

; read asi image.
mofn = sdiskdir('Research')+'/sdata/themis/thg/mosaic/thg_asf_mosaic_2013_0414_0400_to_1000.cdf'
moid = cdf_open(mofn)
...
uts = time_double(['2013-04-14/08:05','2013-04-14/08:05:03'])
;uts = time_double(['2013-04-14/07:11','2013-04-14/07:11:03'])
