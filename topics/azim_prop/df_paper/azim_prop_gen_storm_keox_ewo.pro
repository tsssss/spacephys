...
;---Preparation.
    ...
    if n_elements(xsm_range) ne 2 then xsm_range = [-13,4]
...
;---Generate the plot.
    ...
    file = join_path([homedir(),'fig_azim_prop_storm_keox_ewo_'+time_string(time_range[0],tformat='YYYY_MMDD')+'.pdf'])
    ...
    labels = letters(npanel)+'. '+['Dst','AE','UT-MLT','UT-X!USM']
