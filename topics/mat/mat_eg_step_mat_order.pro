; Compare cwt Guassian wavelet and mat at various orders,
; using test function f0 = fi+f3+f5+..., truncated at nterms.
;
; It shows that Guassian wavelet has similar result with
; order-0 mat. As larger orders, bands are well seperated.

    ofn = shomedir()+'/mat_step.pdf'
    ofn = 0
    ...
    f0 = dblarr(nx)
    for i = 0, np-1 do f0 += 4/!dpi/(2*i+1)*sin((2*i+1)*x)
    ...
    plot, xr, yr, /nodata, /noerase, position = tpos, /normal, color = cgrid, $
        ...
        yrange = yr, ystyle = 1, ylog = 0, yticks = yticks, yminor = 0, yticklen = 0, ygridstyle = gstyle, ytickformat = '(A1)'
