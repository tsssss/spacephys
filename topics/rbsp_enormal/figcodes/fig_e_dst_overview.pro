...
; load dst.
    ...
    if tnames(tvar) eq '' then begin
        ...
        store_data, tvar, uts, dat.sym_h, limits = $
            {constant:[0,symh0], ytitle:'Sym/H!C(nT)'}
...
; load the times for all events.
    ...
    prbs = strarr(ninfo)
    utrs = transpose(infos.utr)
    for i = 0, ninfo-1 do prbs[i] = strmid(infos[i].id,0,1,/reverse)

    ; save event times to tplot.
...
;
;vars = ['symh','rb_apo_mlt','storm_cnt_norm','rbspa_utrs','rbspb_utrs']
...
; **** plot for the pre-midnight sector.
ofn = 0
ofn = shomedir()+'/fig_e_dst_overview.pdf'
