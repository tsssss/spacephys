...
function micro_injection_fig_ulf_v04, input_event_id, probe=probe, $
...
;---Plot settings.
    ...
    pansize = [6,0.75]
    margins = [10,4,10,1]
    poss = panel_pos(plot_file, nypan=nplot_var, $
        pansize=pansize, fig_size=fig_size, margins=margins)
    
    plot_tr = time_double(['2015-09-01/18:00','2015-09-01/21:30'])
    ...
    options, plot_vars, xticklen=xticklen, yticklen=yticklen
    
    
    tplot_options, 'tickinterval', 1200
    my_pos = poss
    bottom_vars = plot_vars
    my_letters = fig_letters
    my_labels = panel_labels
    ...
    foreach var, bottom_vars, vid do begin
        ...
        options, var, xticklen=xticklen, yticklen=yticklen, ytickformat=''
    endforeach
    
    
    tplot, bottom_vars, position=my_pos, noerase=1, trange=plot_tr
    foreach var, bottom_vars, vid do begin
        tpos = my_pos[*,vid]

        msg = my_letters[vid]+') '+my_labels[vid]
...
print, micro_injection_fig_ulf_v04(event_id, probe=probe, test=0, update=1)
end