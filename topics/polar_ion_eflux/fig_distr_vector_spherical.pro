...
pro fig_distr_vector_spherical, dattype, bintype, $
...
;---Settings.
    ...
    case dattype of
        'h_bulk_velocity': begin
            unit = 'H+ V!D||!N (km/s)'
            zrang = [-1,1]*30
            end
        'o_bulk_velocity': begin
            unit = 'O+ V!D||!N (km/s)'
            zrang = [-1,1]*20
            end
    endcase
...
;---Plot.
    ...
    fig_distr_vector_spherical, dattype, bintype, bininfos=bininfos, tdat=tdat, bincnts=bincnts, /noplot, slicerng=slicerng-12
    for i=0,tsz[0]-1 do for j=0,tsz[1]-1 do begin
        tc = tdat[i,j]
        if bincnts[i,j] le mincnt then continue
...
foreach tvar, vars do fig_distr_vector_spherical, tvar
end
