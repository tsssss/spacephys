...
;---settings.
    utr0 = time_double(['2013-06-07/04:45','2013-06-07/05:15'])  ; long time range.
    utr1 = utr0+[-1,1]*300   ; 5 min pad time.
    utr2 = time_double(['2013-06-07/04:52','2013-06-07/05:02'])  ; short time range.
    utr3 = time_double(['2013-06-07/04:54','2013-06-07/04:57'])  ; ewo time range.
    utr4 = time_double(['2013-06-07/04:45','2013-06-07/05:05'])  ; keo time range.


    rootdir = sdiskdir('GoogleDrive')+'/My Drive/works/works/rbsp_psbl1'
    datdir = rootdir+'/data'
    if file_test(datdir,/directory) eq 0 then file_mkdir, datdir
    datfn = datdir+'/2013_0607_all_data.tplot'
...
;---One-time settings.
    ...
    ; mos vars.
    mosvars = ['asf_'+['mos','info'], pres+'keo_mos','mos_ewo_'+ewo_lats]
    ...
    momvar1s = ['vbulk','number_flux','energy_flux','enthalpy']
    tvar = ['density','t_avg',momvar1s+'_gsm',momvar1s+'_fac']
    ions = ['p','o','he']
    species = ['e',ions]
    momvars = []
    foreach pre0, pres do foreach tspecie, species do momvars = [momvars,pre0+tspecie+'_'+tvar]

    ; all vars.
    allvars = [infovars,posvars,mapvars,ebvars,pfvars,esavars,asivars,mosvars,momvars]
...
;---event info.
    ...
    if load then begin
        dfuts = time_double(['2013-06-07/04:54:32','2013-06-07/04:54:59'])  ; DF time at -A and -B.
        info0 = {dfut:0d, fmlat:dblarr(nmodel), fmlt:dblarr(nmodel)}
        tid = time_string(utr0[0],tformat='YYYY_MMDD')
        einfo = {id: tid, model: models, filter: filter, scaleinfo: scaleinfo}
        ...
        foreach tprobe, probes, i do begin
            ...
            print, 'Footpoint MLT (hr):   ', sgnum2str(mean(tfmlts)+24,ndec=1), '+/-', sgnum2str(stddev(tfmlts),ndec=1)
...
; load E/B field, separate B0 and dB from B.
    ...
    if load then begin
        ...
        foreach tprobe, probes do begin
            ...
            ; load B.
            rbsp_read_bfield, utr0, probe=tprobe
            get_data, 'rbsp'+tprobe+'_b_gsm', tuts, bgsm
    ...
    if load then begin
        foreach tprobe, probes do begin
            ...
            stplot_calc_pflux_mor, pre0+'de_fac', pre0+'db_fac', pre0+'pf_fac', $
                scaleinfo=scaleinfo, filter=filter
...
;---Load mos.
    ...
    load = 0
    foreach tvar, vars do if tnames(tvar) eq '' then load = 1
    if keyword_set(reload_mos) then load = 1

    if load then begin
        asifn = sread_thg_mosaic(utr4, sites, type='asf', minlat=minlat, dark=0, /notop)
        cdfs = scdfread(asifn)
        ...
        store_data, 'asf_mos', uts[idx], mos[idx,*], pxidx
        store_data, 'asf_info', 0, {imgsz:imgsz, minlat:minlat, mlats:mlats, mlts:mlts}

        ; calc keogram.
        tmlts = mlts/15  ; convert to hr [-12,12].
        get_data, 'event_info', 0, einfo
        vnames = strlowcase(tag_names(einfo))
        foreach pre0, pres, k do begin
            tidx = where(vnames eq pre0)
        ...
        ; calc ewogram.
        get_data, 'asf_mos', uts, mos
        idx = where(uts ge utr3[0] and uts le utr3[1])
        uts = uts[idx]
        mos = mos[idx,*]
        nrec = n_elements(uts)
        tmlts = mlts/15  ; convert to hr [-12,12].
...
;---load moment data.
    ...
    load = 0
    foreach tvar, vars do if tnames(tvar) eq '' then load = 1
    if keyword_set(reload_mom) then load = 1

    if load then begin
        units = ['(cm!U-3!N)','(eV)','(km/s)','(#/cm!U-2-s)','(mW/m!U2!N)','(mW/m!U2!N)']
        foreach tprobe, probes do begin
            pre0 = 'rb'+tprobe+'_'
            ;moms = sread_rbsp_hope_moments(utr0, probe=tprobe)
            ofn = shomedir()+'/'+pre0+'moments.cdf'
            rbsp_gen_hope_moments, probe=tprobe, utr0, ion_energy_range=[30,1e4], filename=ofn
            ...
            foreach tspecie,species do begin
                ...
                foreach tvar, momvar0s, i do begin
                    ...
                    store_data, ttvar, uts, dat, limits={ytitle:tspecie+' '+tvar+'!C'+units[i]}
                endforeach
