;+
; Plot AE, ewograms of DP, upward current, and downward current.
;-

pro fig_ewogram_of_dp_and_up_down_current, time_range, probes=probes, $
...
;---Get the size of the figure, and position of the panels.
    ...
    fig_labels = letters(nypanel)+'. '+['AE','Dipolari-!C     zation','Upwd!C    current','Downwd!C    current']
    ypad = 0.4
...
;---AE.
    ...
    if n_elements(ytickv) eq 1 then ytickv = make_bins(yrange, ystep)
...
fig_ewogram_of_dp_and_up_down_current, time_range, probes=probes, test=1, mlt_range=mlt_range
end
