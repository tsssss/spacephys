...
;---Check inputs.
    ...
    if n_elements(long_time_range) ne 2 then begin
        errmsg = handle_error('No input long_time_range ...')
        return
    endif
...
;---Get the size of the figure, and position of the panels.
    ...
    ; First (upper) 2 panels.
    upper_ypans = [0.6,1]
    upper_nypanel = n_elements(upper_ypans)
    upper_panel_ypad = 0.4
    upper_panel_xsize = panel_xsize+panel_xpad*abs_xchsz+right_pan_xsize
    upper_panel_ysize = panel_ysize
    upper_pan_ysize = upper_panel_ysize*total(upper_ypans)+upper_panel_ypad*(upper_nypanel-1)*abs_ychsz
    upper_ypad = 4

    fig_ysize = right_pan_ysize+total(margins[[1,3]])*abs_ychsz+$
        upper_ypad*abs_ychsz+upper_pan_ysize
    ...
    file = join_path([project.plot_dir,'fig_'+event_id+'_timing_result.pdf'])
    ...
    tposs = sgcalcpos(2, position=pos, ypans=[upper_pan_ysize,right_pan_ysize], ypad=upper_ypad)
    upper_pos = tposs[*,0]
    upper_poss = sgcalcpos(upper_nypanel, position=upper_pos, ypans=upper_ypans, ypad=upper_panel_ypad)
...
;---Panel b. UT-MLT long.
    ...
    oplot, short_time_range, yrange[0]+[0,0], color=sgcolor('black'), thick=bar_thick
    tmp = convert_coord(short_time_range[0], yrange[0], /data, /to_normal)
    ttpos = left_poss[*,0]
    plots, [tmp[0],ttpos[0]], [tmp[1],ttpos[3]], /normal, linestyle=0
    tmp = convert_coord(short_time_range[1], yrange[0], /data, /to_normal)
    ttpos = left_poss[*,0]
    plots, [tmp[0],ttpos[2]], [tmp[1],ttpos[3]], /normal, linestyle=0

    tx = label_xshift*xchsz
    ty = tpos[3]-label_yshift*ychsz
    xyouts, tx,ty,/normal, 'b. UT-'+strmid(ytitle,0,strpos(ytitle,' '))+'!C    Overview'
    ...
    foreach drift_time, drift_times, drift_id do begin
        ...
        xyouts, mean(drift_time_range), label_ypos,/data, alignment=0.5, string(drift_id+1,format='(I0)');, charsize=label_size
    endforeach

;    ; Add all large DF in the time range.
;    large_dfs = azim_df_search_event(project=project, /get_large_df_events)
;    large_df_count = 0
;    large_df_color = sgcolor('grey')
;    normal_ypos = tpos[3]-ychsz*0.4
;    data_ypos = (convert_coord(0,normal_ypos, /normal, /to_data))[1]
;    foreach large_df, large_dfs do begin
;        if product(large_df.time_range-long_time_range) gt 0 then continue
;        txs = large_df.time_range
;        tys = data_ypos
;        plots, txs,tys,/data, color=large_df_color
;        foreach df, large_df.df_list do begin
;            if product(long_time_range-df.arrival_time) gt 0 then continue
;            tx = df.arrival_time
;            tmp = convert_coord(tx,0, /data, /to_normal)
;            txs = tmp[0]+[0,0]
;            tys = normal_ypos+[-1,1]*ychsz*0.1
;            plots, txs,tys,/normal, color=large_df_color
;            large_df_count += 1
;        endforeach
;    endforeach
;    xyouts, tpos[0]+xchsz*5, normal_ypos-ychsz*0.25, /normal, string(large_df_count,format='(I0)')+' DFs', charsize=0.7, color=large_df_color
...
;---Panel c. UT-MLT short.
    ...
    foreach df, df_list, ii do begin
        plots, df.obs_time,df.obs_mlt,/data, psym=6, symsize=0.5
    endforeach
    tx = tpos[0]+xchsz*1
    ty = tpos[3]-ychsz*1.2
...
;---UT-X.
    ...
    foreach df, df_list do begin
        ...
        data_var = prefix+data_var_suffix
        zzs = get_var_data(data_var, in=short_time_range, times=xxs)
        yys = (get_var_data(pos_var, at=xxs))[*,0]
        mlt = get_var_data(prefix+'pseudo_mlt', at=xxs)
        zzs = azim_df_normalize_theta(zzs, zrange=tilt_range, ct=ct, /reverse_ct)
    ...
    foreach df, df_list, ii do begin
;        tmp = convert_coord(df.obs_time,df.obs_r_sm[0], /data, /to_normal)
;        txs = tmp[0]+[0,0]
;        tys = tmp[1]+[-1,1]*ychsz*0.5
;        plots, txs,tys,/normal
        plots, df.obs_time,df.obs_r_sm[0],/data, psym=6, symsize=0.5;, color=probe_colors[ii], thick=square_thick
    endforeach
    ...
    tx = label_xshift*xchsz
    ty = tpos[3]-label_yshift*ychsz
    xyouts, tx,ty,/normal, 'd. UT-'+strmid(ytitle,0,strpos(ytitle,' '))
...
;        yrange = (yrange[1]+yrange[0])*0.5+[-1,1]*(yrange[1]-yrange[0])*0.75
    ...
    tx = label_xshift*xchsz
    xyouts, tx,ty_label,/normal, 'e. Detrended '+tex2str('theta')+', time lag removed'
...
;---Draw shape.
    ...
    plots, tmp*cos(angles[0]), tmp*sin(angles[0]), /data
    ...
    xyouts, tx,ty,/normal, 'W = '+string(ww_angle*deg,format='(I0)')+' deg!C  ~ '+sgnum2str(ww_center,ndec=1)+' Re', charsize=label_size
...
;---Draw v_2d and triads.
    ...
    foreach triad, triad_list do begin
        ...
        x1 = x0+v_hat[0]*scale
        y1 = y0+v_hat[1]*scale
        arrow, x0,y0,x1,y1,/data, solid=arrow_solid, hsize=arrow_hsize*1.5, color=sgcolor('gray')
    endforeach
