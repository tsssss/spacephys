...
function fig_rbsp_eflux_eg_v01, plot_dir, event_info=event_info, test=test
...
;        mlts = aacgm_mlon2mlt(mlons, the_times)
    ...
    foreach probe, rbsp_probes do begin
        prefix = 'rbsp'+probe+'_'
        plot_vars = [plot_vars, prefix+['o_en_spec','o_pa_spec','eflux_combo','lshell']]
    endforeach
    ...
    ypans = []
    foreach probe, rbsp_probes do begin
        the_labels = ['O+ EN','O+ PA','Eflux','L-shell']
...
;    if n_elements(plot_dir) eq 0 then plot_dir = event_info.plot_dir
    ...
    plot_file = join_path([plot_dir,'fig_rbsp_eflux_eg_'+id+'_'+version+'.pdf'])
    ...
    timebar, pp_times, linestyle=2, color=sgcolor('white')
    
    the_color = sgcolor('white')
    pid = 1
    tpos = poss[*,pid]
    set_axis, plot_vars[pid], position=tpos
    msg = 'Auroral oval conjunction'
    hsize = (keyword_set(test))? 8: 160

    tmp = convert_coord(mean(pp_times),0, data=1, to_normal=1)
    tx = tmp[0]
    ty = mean(tpos[[1,3]])
    xyouts, tx,ty-ychsz*0.2,msg, normal=1, color=the_color, alignment=0.5
    len = strlen(msg)*0.5
    tx0 = tx-xchsz*len
    tmp = convert_coord(pp_times[0],0, data=1, to_normal=1)
    tx1 = tmp[0]
    arrow, tx0,ty,tx1,ty, hsize=hsize, solid=1, normal=1, color=the_color
    tx0 = tx+xchsz*len
        tmp = convert_coord(pp_times[1],0, data=1, to_normal=1)
    tx1 = tmp[0]
    arrow, tx0,ty,tx1,ty, hsize=hsize, solid=1, normal=1, color=the_color
...
print, fig_rbsp_eflux_eg_v01(event_info=event_info, test=0)
end