...
pro fig_distr_scalar_spherical_hydra, dattype, bintype, $
...
;---Settings.
    ...
    case dattype of
        'DENSITY_ELE': begin
            unit = 'Log10 Ele density (cm!U-3!N)'
            zrang = [-1,2]
            end
        'DENSITY_ION': begin
            unit = 'Log10 Ion density (cm!U-3!N)'
            zrang = [-1,2]
            end
        'TPERP_ION': begin
            unit = 'Log10 Ion Tperp (eV)'
            zrang = [2.5,4]
            end
        'TPARL_ION': begin
            unit = 'Log10 Ion Tpara (eV)'
            zrang = [2.5,4]
            end
    endcase
    ...
    xsz = 6d ; inch
    ysz = abs((yrang[1]-yrang[0])/(xrang[1]-xrang[0])*xsz)+1

    ; filenames.
    stryrs = string(smkarthm(1996,2008,1,'dx'),format='(I4)')
    stryrs = stryrs[where(stryrs ne '2006')]
    nyr = n_elements(stryrs)
    rootdir = shomedir()+'/polar_ion_eflux'
    datadir = shomedir()+'/polar_ion_eflux/data'
    binfns = datadir+'/'+stryrs+'/polar_ion_eflux_bin_info_hydra_'+bintype+'_'+stryrs+'.sav'
    datfns = datadir+'/'+stryrs+'/polar_ion_eflux_hydra_'+dattype+'_'+stryrs+'.sav'
    utsfns = datadir+'/'+stryrs+'/polar_ion_eflux_uts_hydra_'+stryrs+'.sav'

    figfn = shomedir()+'/fig_distr_hydra_'+dattype+'_'+bintype+'.pdf'
...
;    stop
    ...
    fig_distr_scalar_spherical_hydra, dattype, bintype, bininfos=bininfos, tdat=tdat, bincnts=bincnts, /noplot, slicerng=slicerng-12
    for i=0,tsz[0]-1 do for j=0,tsz[1]-1 do begin
        tc = tdat[i,j]
        if bincnts[i,j] le mincnt then continue
    ...
    sgcolorbar, findgen(top), /horizontal, position=pos1, /noerase, ct=ct0, $
        ...
        ztickv=ztickv, ztickname=ztickn, zticks=nztick-1
...
vars = ['DENSITY_ION','DENSITY_ELE','TPARL_ION','TPERP_ION']
vars = ['DENSITY_ION','DENSITY_ELE']
foreach tvar, vars do fig_distr_scalar_spherical_hydra, tvar
end
