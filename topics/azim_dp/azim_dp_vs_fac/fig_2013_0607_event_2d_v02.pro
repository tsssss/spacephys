...
;times = time_double('2013-06-07/'+['04:47','04:59','05:21'])
fig_label = 'f-'
...
plot_file = join_path([homedir(),'Dropbox','mypapers','dp_vs_fac','plot',$
    'fig_'+strjoin(time_string(time_range,tformat='YYYY_MMDD_hhmm'),'_')+'_2_v02.pdf'])
...
;        xyouts, tx,ty, /normal, time_string(time)
    ...
    cbpos[3] = poss[3,0,0]
    cbpos[0] = cbpos[2]+xpad*xchsz
    cbpos[2] = cbpos[0]+cb_width
    sgcolorbar, reverse(make_bins([bottom_color,top_color],1)), ct=ct, reverse_ct=reverse_ct, $
        zrange=zrange, ztitle=' ', position=cbpos
    tx = 1-xchsz*0.5
    ty = cbpos[3]+ychsz*0.4
    msg = ztitle
    xyouts, tx,ty,msg, alignment=1, normal=1, charsize=label_size
