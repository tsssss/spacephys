...
;---Settings.
    test = 0
    test_panels = 0
    plot_file = join_path([homedir(),'Dropbox','mypapers','dp_vs_fac','fig_2014_0828_zoom_in_v02.pdf'])
...
;    xtickn[0] += '!C'+time_string(xtickv[0],tformat='YYYY-MM-DD')
    ...
    foreach probe, 'th'+['d','e','a'], probe_id do begin
        ...
        tx = tpos[2]-xchsz*0.5
        ty = tpos[3]-ychsz*0.9
        xyouts, tx,ty, /normal, alignment=1, 'a-'+string(probe_id+1,format='(I0)')+'. '+strupcase(probe)
    endforeach
    ...
    foreach panel_id, [3,4] do begin
        ...
        xyouts, tx,ty, /normal, alignment=1, 'a-'+string(panel_id+1,format='(I0)')+'. '+the_probe
    endforeach
    tx = tpos[0]-ychsz*1.8
    ty = (mid_poss[1,3]+mid_poss[3,4])*0.5
    msg = lim.ytitle
    xyouts, tx,ty,/normal, alignment=0.5, orientation=90, msg
...
;---Right panel.
    ...
    foreach panel_id, [0,1,2] do begin
        ...
        tx = tpos[0]+xchsz*0.5
        ty = tpos[3]-ychsz*0.9
        xyouts, tx,ty,/normal, 'b-'+string(panel_id+1,format='(I0)')
    endforeach
    tx = tpos[0]-ychsz*1.2
    ...
    tx = tpos[0]+xchsz*0.5
    ty = tpos[3]-ychsz*0.9
    xyouts, tx,ty,/normal, alignment=0, 'b-4'
