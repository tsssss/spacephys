;+
; Check AE to filter events.
;-

pro azim_df_search_df_group_gen_diagnostic_plot, project=project, events, dirname=dirname
...
test = 1
    ...
    foreach event, events do begin
        data_time_range = event.time_range+[1,-1]*constant('secofday')
        plot_time_range = data_time_range+[-1,1]*secofhour*0.5
    ...
    ;---Load data.
        ...
        foreach probe, all_probes do begin
            prefix = probe+'_'
            if check_if_update(prefix+'theta', time_range) then azim_df_read_data, 'theta', time_range=plot_time_range, probe=probe, project=project
            if check_if_update(prefix+'r_sm', time_range) then azim_df_read_data, 'r_sm', time_range=plot_time_range, probe=probe, project=project
    ...
    ;---Size of panel and figure.
        file_suffix = 'azim_df_event_'+strjoin(time_string(data_time_range,tformat='YYYY_MMDD_hhmm'),'_')+'_v01.pdf'
    ...
    ;---Panel 1: SC orbit in XY plane.
        ...
        timing_key = 'df_time'
        if event.haskey('timing_info') then begin
    ...
    ;---MLT-UT.
        ...
        ; Add DF.
        foreach df, event.df_list do begin
            ty = azim_df_calc_pseudo_mlt(df.arrival_r_sm)
            tx = df.arrival_time
            plots, tx,ty,/data, psym=1, symsize=0.5
            tx = df.xcor_time
            plots, tx,ty,/data, psym=6, symsize=0.5
        endforeach
...
dirname = 'sorted_subgroups'

azim_df_search_df_group_gen_diagnostic_plot, events, dirname=dirname, project=project
