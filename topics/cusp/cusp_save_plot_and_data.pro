...
; [po,fa]tr. time range for plots.
; [po,fa]trcusp. cusp entry and exit times.
; [po,fa]filt0. time filters from log file.
; [po,fa]filts. time filters, uniq and sorted from small to large.
; [po,fa]delims. delimiters for low and high freq noises.
; [po,fa]facdelim. delimiter for fac.
    ...
    fatr = fainfo.plot_time
    potrcusp = poinfo.cusp_time
    fatrcusp = fainfo.cusp_time
    ...
    ; fast.
    ; filters, sort from small to large.
    fafilt0 = fainfo.filters     ; original filters.
    fafilts = fafilt0
    fafilts = fafilts[sort(fafilts)]
    fafilts = fafilts[uniq(fafilts)]

    ; delimiters.
    fadelims = fainfo.noisedelim ; below and above are noise.
    tmp = where(fadelims eq -1, cnt)
    if cnt eq 2 then fadelims = minmax(fadelims) ; sort if no -1.
    if fadelims[0] lt 0 then fadelims[0] = 0
    if fadelims[1] lt 0 then fadelims[1] = fatr[1]-fatr[0]

    ; fac limit.
    fafacdelim = fainfo.faclim     ; above is fac.
    if fafacdelim eq -1 then fafacdelim = max(fafilts)

    ; [po,fa]delims. updated to match Polar and FAST phase space.
    autodelims = 0
    if keyword_set(autodelims) then begin
        ; match the high frequency part.
        fadelims[0] = fafilts[0]
        podelims[0] = fafilts[0]*(potr[1]-potr[0])/(fatr[1]-fatr[0])
        ; match the low frequency part.
        fadelims[1] = max(pofilts)
        fadelims[1] = pofacdelim<fadelims[1]      ; polar fac cannot be determined correctly.
        podelims[1] = fadelims[1]<podelims[1]     ; match polar to fast low freq scale.
        podelims[1] = podelims[1]<pofacdelim
    endif else begin
        podelims = [0,max(pofilts)]
        fadelims = [0,max(fafilts)]
    endelse
...
; **** calculate all data and save to tplot.
    ...
    ; [po,fa]_rscl. scales in # of records.
        ...
        store_data, vars, /delete
        get_data, 'fa_de_fac', t0
        fadr = sdatarate(t0)
        tmp = fainfo.scaleinfo
        fatscls = smkgmtrc(tmp[0],tmp[1],tmp[2],'n')
        tmp = round(fatscls/fadr)
        farscls = double(tmp[uniq(tmp)])
        fatscls = farscls*fadr
        fafldnrec = n_elements(t0)
    ...
    ; [po,fa]nband. # of wanted bands.
        ...
        famatfilts = fafilts
        famatfilts = [0,famatfilts,maxfilt]
        fanmatband = n_elements(famatfilts)-1
        famatbandids = '_f'+string(indgen(fanmatband),format='(I0)')
        ...
        stplot_calc_pflux_mat, dename, dbname, pfname, tscale = fatscls, $
        ...
        if keyword_set(remove_wake) then begin
            ...
            dename = 'fa_de_fac_mat'
            dbname = 'fa_db_fac_mat'
            pfname = 'fa_pf_fac_mat'
            vars = [dename,dbname]
            nvar = n_elements(vars)
            for i = 0, nvar-1 do begin
                idx = fanmatband-1
                tvar = vars[i]+famatbandids[idx]
                get_data, tvar, t0, dat
                bg = scalcbg(dat)
                store_data, tvar, t0, dat-bg
                tvar = vars[i]+'_nl'
            ...
            fanmatband+= 1
            famatbandids = [famatbandids,'_nl']

            ; update poynting flux.
            vars = famatbandids[fanmatband-2:*]
            for i = 0, n_elements(vars)-1 do $
                stplot_calc_pflux_mat, dename+vars[i], dbname+vars[i], pfname+vars[i]
            store_data, dename+'_f?_comp'+['1','2','3'], /delete
            store_data, dbname+'_f?_comp'+['1','2','3'], /delete
        ...
        fanband = fanmatband
        fabandids = famatbandids
        fanfilt = fanband
        fabandlabels = strarr(fanband)
        for i = 0, fanband-2 do fabandlabels[i] = $
            sgnum2str(sround(famatfilts[i],error=1e-3))+'-'+sgnum2str(sround(famatfilts[i+1],error=1e-3))+'s'
        fabandlabels[fanband-1] = 'low freq bg'
        ...
        vars = ['fa_'+['ele_keflux','ion_keflux','pf_fac_mat'+['',fabandids]]]
        for j = 0, n_elements(vars)-1 do begin
...
; **** save data to disk.
    ...
    fadevars = 'fa_de_fac_mat'+fanewbandids
    nvar = n_elements(fadevars)
    options, fadevars, 'ystyle', 1
    options, fadevars, 'ytitle', 'dE'
    options, fadevars, 'colors', [6,4,2]
    for i = 0, nvar-1 do begin
    ...
    fadbvars = 'fa_db_fac_mat'+fanewbandids
    nvar = n_elements(fadbvars)
    options, fadbvars, 'labels', faclabs
    options, fadbvars, 'ystyle', 1
    options, fadbvars, 'ytitle', 'dB'
    options, fadbvars, 'colors', [6,4,2]
    for i = 0, nvar-1 do begin
    ...
    fapfvars = 'fa_pf_fac_mat'+fanewbandids
    ...
    options, fapfvars, 'ytitle', 'S'
    options, fapfvars, 'colors', [6,4,2]
    for i = 0, nvar-1 do begin
    ...
    ; mat spectrogram of the wanted component.
    stplot_index, dename, deidx, newname = dename+dematsuf
    stplot_mat, dename+dematsuf, scale = fatscls, zrange = loginfo.fast.de

    vars = dename+'_mat'+fanewbandids
    nvar = n_elements(vars)
    for i = 0, nvar-1 do stplot_index, vars[i], deidx, newname = vars[i]+dematsuf
    ...
    options, tvar, 'zticks', 2
    options, dename+dematsuf, 'labels', 'original!C  dEv'
    tvar = dename+'_mat'+fanewbandids+dematsuf
    ...
    ; mat spectrogram of the wanted component.
    stplot_index, dbname, dbidx, newname = dbname+dbmatsuf
    stplot_mat, dbname+dbmatsuf, scale = fatscls, zrange = loginfo.fast.db

    vars = dbname+'_mat'+fanewbandids
    nvar = n_elements(vars)
    for i = 0, nvar-1 do stplot_index, vars[i], dbidx, newname = vars[i]+dbmatsuf
    ...
    options, tvar, 'zticks', 2
    options, dbname+dbmatsuf, 'labels', 'original!C  dBp'
    tvar = dbname+'_mat'+fanewbandids+dbmatsuf
    for i = 0, n_elements(tvar)-1 do options, tvar[i], 'labels', fanewbandlabels[i]
