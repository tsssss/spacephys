...
pro pflux_grant_read_e_fac, time, probe=probe, id=data_type, $
...
;---Init settings.
    ...
    rbspx = 'rbsp'+probe
    base_name = rbspx+'_efield_l3_%Y_%m%d_'+version+'.cdf'
    local_path = [local_root,rbspx,'efield','l3','%Y']
    ; B field.
    type_dispatch['e_fac'] = dictionary($
        'pattern', dictionary($
...
;---Find files, read variables, and store them in memory.
    ...
    if n_elements(nonexist_files) ne 0 then begin
        foreach file, request.nonexist_files do begin
            ...
            pflux_grant_read_e_fac_gen_file, file_time, probe=probe, filename=local_file
        endforeach
        files = prepare_files(request=request, errmsg=errmsg, local_files=files, $
            file_times=file_times, time=time, nonexist_files=nonexist_files)
    endif
...
;---Read data from files and save to memory.
    ...
    foreach var, prefix+['e','edot0'] do begin
        ...
        settings = {$
            ...
            coord: 'FAC', $
            coord_labels: fac }
        if var eq prefix+'edot0' then settings.short_name = 'dot0 E'
        add_setting, e_var, /smart, settings
    endforeach
...
pflux_grant_read_e_fac, time, probe=probe
...
foreach probe, probes do begin
    ...
    foreach day, days do begin
        ...
        pflux_grant_read_e_fac, time_range, probe=probe
    endforeach
endforeach
end
