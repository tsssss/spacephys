...
; low_energy_var.
;-
function calc_phase_diff, high_energy_var, low_energy_var

    prefix = get_prefix(high_energy_var)
    phase_diff_var = prefix+'phase_diff'
    time_range = get_var_setting(high_energy_var, 'requested_time_range')
    if ~check_if_update(phase_diff_var, time_range) then begin
        return, phase_diff_var
    endif

    mora = get_var_data(high_energy_var, times=times, freqs)
    morb = get_var_data(low_energy_var)
    morab = mora*conj(morb)
    phase = atan(imaginary(morab)/real_part(morab))*constant('deg')

    phase_var = prefix+'cross_phase'
    unit = 'deg'
    phase_var = var_store(phase_var, phase, times, freqs*1e3, settings=dictionary($
        'ytitle', 'Freq!C(mHz)', $
        'display_type', 'spec', $
        'ztitle', 'Phase ('+unit+')', $
        'zlog', 0, $
    ...
    get_data, phase_var, times, mors, freqs
    project_info = micro_injection_stat_load_project()
    freq_range = project_info['freq_range']*1e3
    ...
    store_data, phase_diff_var, times, phase_diff
    ytickv = [-1,0,1]*50
    yrange = [-1,1]*70
    add_setting, phase_diff_var, smart=1, dictionary($
        'display_type', 'scalar', $
...
function micro_injection_spatial_distribution_phase_diff_simplest_v01, mission_probe, test=test
...
;---Settings.
    ...
    plot_file = join_path([plot_dir,'micro_injection_spatial_distribution_'+mission_probe_str+'_phase_diff_'+version+'.pdf'])
...
;---Load data and collect phase diff.
    ...
    phase_avg_var = prefix+'spatial_bin_'+bin_type+'_phase_diff'
    if check_if_update(phase_avg_var) then begin
        phase_avgs = fltarr(nmlt_bin,nmlat_bin,ndis_bin)
        phase_counts = fltarr(nmlt_bin,nmlat_bin,ndis_bin)
        ...
        ;---Load flux and phase.
            ...
            high_index = where(stregex(spec_vars,high_energy_str) ne -1, complement=low_index)
            high_energy_var = spec_vars[high_index]
            low_energy_var = spec_vars[low_index]
            options, spec_vars, requested_time_range=tr
            phase_diff_var = calc_phase_diff(high_energy_var, low_energy_var)
        ...
        ;---Collect results.
            ...
            phase_diffs = get_var_data(phase_diff_var, at=mi_times)

            foreach time, mi_times, tid do begin
                phase_counts[mlt_index[tid],mlat_index[tid],dis_index[tid]] += 1
                phase_avgs[mlt_index[tid],mlat_index[tid],dis_index[tid]] += phase_diffs[tid]
            endforeach
        endfor

        phase_avgs = phase_avgs/phase_counts
        phase_avg_var = var_store(phase_avg_var, phase_avgs, 0)
    endif
    phase_avgs = get_var_data(phase_avg_var)
...
;---XY plane.
    ...
    zzs = total(phase_avgs,2,nan=1)
    if keyword_set(zlog) then begin
        zrange = alog10(phase_zrange)
...
;---color bar.
    ...
    sgcolorbar, findgen(top_color), ct=ct, position=cbpos, horizontal=cb_hor, ztitle=ztitle, $
        zrange=phase_zrange, zticklen=zticklen, zcharsize=0.9
...
test = 0
print, micro_injection_spatial_distribution_phase_diff_simplest_v01(test=test)
end