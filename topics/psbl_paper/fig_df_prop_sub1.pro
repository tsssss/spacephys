...
;---Construct an event.
    event_id = time_string(mean(time_range),tformat='YYYY_MMDD_hhmm')
...
;---Linear fit.
    ...
    foreach df, df_list, ii do begin
        df_times[ii] = df.arrival_time
        ;df_times[ii] = df.xcor_time
    ...
    event['omega_linear_fit'] = linear_fit_slope
    event['linear_fit_r_square'] = linear_fit_r_square
...
;---Xcor.
    ...
    event = azim_df_analysis_event(event, project=project, /no_load_data)
    timing_info = event.timing_info.combined_time
...
;---Get the size of the figure, and position of the panels.
    ...
    file = join_path([srootdir(),'fig_df_prop_sub1.pdf'])
...
;---Panel b. UT-MLT short.
    ...
    ; Linear-fit result.
    linear_fit_info = timing_info.linear_fit_info
    fit_result = linear_fit_info.fit_result
    ...
    arrival_times = linear_fit_info.ref_times
    arrival_mlts = linear_fit_info.ref_mlts
    ...
    tx = tpos[0]+xchsz*1
    ty = tpos[3]-ychsz*1.2
    omega_fit = linear_fit_info.omega_fit
    r_square = linear_fit_info.r_square
...
;    combo_probes = list()
    foreach probe_combo, probe_combos do begin
        ...
        omegas.add, combo_info.omega_triad
;        combo_probes.add, strsplit(probe_combo,'_',/extract), /extract
    ...
    ref_rsms = timing_info.ref_rsms
    probes = timing_info.probes
    foreach probe, probes, ii do begin
        ;index = where(combo_probes eq probe, count)
        ;if count eq 0 then continue
        rsm = reform(ref_rsms[ii,*])
