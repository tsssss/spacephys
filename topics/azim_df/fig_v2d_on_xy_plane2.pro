...
test = 0
    ...
    file = join_path([project.plot_dir, 'fig_v2d_on_xy_plane2.pdf'])
...
;---Plot settings.
    ...
    foreach event, events do begin
        ...
        the_mlt_range = minmax(obs_mlts)
        the_rxy_range = minmax(obs_rxys)
        mean_mlt = mean(the_mlt_range)
        mean_rxy = mean(the_rxy_range)
        ; Rxy range.
        ;the_mlt = min(the_mlt_range,/abs)
        the_mlt = mean_mlt
        the_angle = (the_mlt*15+180)*rad
        txs = cos(the_angle)*the_rxy_range
        tys = sin(the_angle)*the_rxy_range
        plots, txs,tys, color=region_color
        ; MLT range.
        the_rxy = mean_rxy
        the_angle_range = (the_mlt_range*15+180)*rad
        the_angles = smkarthm(the_angle_range[0],the_angle_range[1],20,'n')
        txs = cos(the_angles)*the_rxy
        tys = sin(the_angles)*the_rxy
        plots, txs,tys, color=region_color
    endforeach

    ; Add arrow.
    keys = ['eastward','westward','earthward','outward']
    colors = sgcolor(['blue','green','red','purple'])
    tx = tpos[0]+xchsz*1
    ty_label = tpos[1]+ychsz*2.;3.
    foreach key, keys, key_id do begin
    ...
    foreach event, events, event_id do begin
        ...
        foreach triad, event.triad_list do begin
            ...
            color = (colors[where(keys eq direction)])[0]
