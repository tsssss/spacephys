...
pro rbsp_hope_l2_balance_pixel_flux, hope, pxs, type = vname0, index = enidx, coefs
    ...
    for j = 0, nen-1 do begin
        ...
        ; balancing the opposite pixels.
        if px1 ne px2 then begin
            v1 = (f2+f1)*0.5
            v2 = (f2-f1)*0.5
            idx = where(v1 ge max(v1)/10)
            
            avg = mean(f1[idx]/f2[idx])
            coef = (1+avg)/(1-avg)
            f1 = v1-v2*coef
            f2 = v1+v2*coef
            
            coefs = [1d,coef]
        endif else coefs = [1d,1d]
