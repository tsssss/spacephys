...
;---Read keV e- flux.
    ...
    if check_if_update(probes[0]+'_kev_e_flux') then begin
        foreach probe, ['b'] do rbsp_read_kev_electron, time_range, probe=probe
        foreach probe, ['a','d','e'] do themis_read_kev_electron, time_range, probe=probe
        foreach probe, ['13','15'] do goes_read_kev_electron, time_range, probe=probe
    endif
