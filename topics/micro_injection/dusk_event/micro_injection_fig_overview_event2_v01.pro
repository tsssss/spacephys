...
; change Ele to e-.
;-
function micro_injection_fig_overview_event2_v01, input_event_id, probe=probe, $
    ...
    time_range = time_double(['2015-09-05/14:00','2015-09-05/16:00'])
...
;---Load data.
    ...
    ; To force read v01.
    var = lets_read_this(func='mms_read_pad_ele_kev', $
        time_range, probe=mission_probe, errmsg=errmsg)
    var = lets_read_this(func='mms_read_pad_ion_kev', $
        time_range, probe=mission_probe, errmsg=errmsg)

    ; Particle related vars.
    ele_pad_var = lets_read_this(func='mms_read_pad_ele_thermal', $
        time_range, probe=mission_probe, errmsg=errmsg)
...
;---Derived data.
    suffix = '_'+['thermal','kev']
;    pad_vars = prefix+['e_pad'+suffix,'p_pad'+suffix]
    pad_vars = prefix+['e_pad'+suffix]
...
;---Load more data
    ...
    vars = prefix+['b_mms_fac','e_mms_fac','vexb_mms_fac','u_mms_fac']
    options, vars, 'labels', fac_labels
    
    var = prefix+'b_mms_fac'
    options, var, yrange=[-20,50], ytickv=[0,20,40], yminor=4, yticks=2, constant=[0,20,40]
    var = prefix+'e_mms_fac'
    options, var, yrange=[-1,1]*3.5, ytickv=[-1,0,1]*3, yminor=3, yticks=2, constant=[0]
    var = prefix+['u_mms_fac','vexb_mms_fac']
    options, var, yrange=[-1,1]*230, ytickv=[-1,0,1]*200, yminor=5, yticks=2, constant=[-1,0,1]*200
...
;        'e_pa_spec_kev','e_pa_spec_thermal_high','e_pa_spec_thermal_low', $
        'u_mms_fac']
;        'b_mms_fac','u_mms_fac','vexb_mms_fac']
    panel_labels = [$
        'e- EN high','e- EN',$
...
probe = '4'
event_id = '2015_0911_18'
probe = '1'
event_id = '2015_0905_14'
print, micro_injection_fig_overview_event2_v01(event_id, probe=probe, test=0, update=1)
end